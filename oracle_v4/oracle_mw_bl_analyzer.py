# oracle_mw_bl_analyzer.py — воркер BL-анализатора (v5, post-WL): счётчик BL-соответствий, кривая ROI(T), выбор T*, история и сглаженный порог в active
...
# 🔸 Логгер
log = logging.getLogger("ORACLE_MW_BL_ANALYZER")

# 🔸 Константы воркера / стримы
MW_WL_READY_STREAM = "oracle:mw_whitelist:reports_ready"   # источник событий (v5 ready)
MW_WL_CONSUMER_GROUP = "oracle_mw_bl_analyzer_group"
MW_WL_CONSUMER_NAME  = "oracle_mw_bl_analyzer_worker"
READ_COUNT = 128
...
async def run_oracle_mw_bl_analyzer():
    # условия достаточности окружения
    if infra.pg_pool is None or infra.redis_client is None:
        log.debug("❌ Пропуск BL_ANALYZER: PG/Redis не инициализированы")
        return
    ...
    try:
        await infra.redis_client.xgroup_create(
            ...
            groupname=MW_WL_CONSUMER_GROUP,
            id="$",
            mkstream=True,
        )
        log.debug("📡 Создана consumer group для %s", MW_WL_READY_STREAM)
    ...
    log.info("🚀 Старт oracle_mw_bl_analyzer (v5, post-WL)")
    ...
    while True:
        try:
            resp = await infra.redis_client.xreadgroup(
                groupname=MW_WL_CONSUMER_GROUP,
                consumername=MW_WL_CONSUMER_NAME,
                ...
                for _stream_name, msgs in resp:
                    for msg_id, fields in msgs:
                        try:
                            ...
                            # стратегия должна быть в MW (по кэшу)
                            ...
                            # гард «последний 7d» — не обрабатываем устаревшие окна
                            ...
                            log.exception("❌ Ошибка обработки сообщения BL_ANALYZER")
                            acks.append(msg_id)

            # ACK обработанных сообщений
            if acks:
                try:
                    await infra.redis_client.xack(MW_WL_READY_STREAM, MW_WL_CONSUMER_GROUP, *acks)
                except Exception:
                    log.exception("⚠️ Ошибка ACK в BL_ANALYZЕР")

        except asyncio.CancelledError:
            log.debug("⏹️ BL_ANALYZER остановлен по сигналу")
            raise
        except Exception:
            log.exception("❌ Ошибка цикла BL_ANALYZЕР — пауза 5 секунд")
    ...
    for tf in TF_LIST:
        for direction in DIRECTIONS:
            try:
                ...
                log.exception("❌ Ошибка sid=%s tf=%s dir=%s", strategy_id, tf, direction)


# 🔸 Расчёт по паре (TF,DIR): выборка post-WL, кривая ROI(T), запись истории и active
async def _process_pair_tf_dir(
    ...
    rows = await conn.fetch(
        ...
        wl_match AS (
          ...
          FROM base_p p
          JOIN oracle_mw_positions omp ON omp.position_uid = p.position_uid
          JOIN oracle_mw_dict md       ON md.id = omp.mw_dict_id
          JOIN oracle_mw_whitelist wwl ON wwl.aggregated_id IS NOT NULL
                                      AND wwl.version = 'v5' AND wwl.list = 'whitelist'
                                      AND wwl.strategy_id = $1
                                      AND wwl.timeframe   = $5
                                      AND wwl.direction   = $2
                                      AND wwl.agg_base = md.agg_base
                                      AND wwl.agg_state = md.agg_state
          WHERE md.strategy_id = $1 AND md.timeframe = $5 AND md.direction = $2
        ),
        passed AS (
          SELECT p.position_uid, p.pnl
          FROM base_p p
        ...
        bl_counts AS (
          ...
          FROM passed pa
          LEFT JOIN oracle_mw_positions omp ON omp.position_uid = pa.position_uid
          LEFT JOIN oracle_mw_dict md       ON md.id = omp.mw_dict_id
          LEFT JOIN oracle_mw_whitelist wbl ON wbl.version = 'v5' AND wbl.list = 'blacklist'
                                           AND wbl.strategy_id = $1
                                           AND wbl.timeframe   = $5
                                           AND wbl.direction   = $2
                                           AND md.strategy_id  = $1
                                           AND md.timeframe    = $5
                                           AND md.direction    = $2
                                           AND wbl.agg_base = md.agg_base
                                           AND wbl.agg_state = md.agg_state
    ...
    if not rows:
        ...
        log.debug("🧹 BL_ACTIVE удалён (нет post-WL позиций): sid=%s tf=%s dir=%s", strategy_id, tf, direction)
        return
    ...
    log.debug("✅ BL sid=%s tf=%s dir=%s → T*=%d (smooth=%d) ROI=%.6f base=%.6f n=%d",
...
async def _insert_history(
    *,
    ...
    await conn.execute(
        """
        INSERT INTO oracle_mw_bl_analysis (
...
async def _upsert_active(
    *,
    ...
    await conn.execute(
        """
        INSERT INTO oracle_mw_bl_active (
...
async def _delete_active_if_exists(conn, strategy_id: int, tf: str, direction: str):
    await conn.execute(
        """
        DELETE FROM oracle_mw_bl_active
...
async def _compute_smoothed_threshold(conn, strategy_id: int, tf: str, direction: str, T_curr: int) -> Tuple[int, int, Dict]:
    ...
    items = sorted(hist, key=lambda x: (x[0],))  # сортируем по T
