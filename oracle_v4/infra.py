...
def set_enabled_tickers(new_dict: dict):
    """
    new_dict: {symbol -> row_dict_from_db}
    """
...
def set_market_watcher_strategies(id_set: set[int]):
    """
    id_set: множество ID стратегий, у которых market_watcher=true
    """
...
# 🔸 Точечные операции с кэшем стратегий
def add_market_watcher_strategy(sid: int):
    market_watcher_strategies.add(int(sid))
    log.debug(
        "Добавлена стратегия в кэш market_watcher: %s (итого %d)",
        sid, len(market_watcher_strategies)
    )
...
def remove_market_watcher_strategy(sid: int):
    ...
    log.debug(
        "Удалена стратегия из кэша market_watcher: %s (итого %d)",
        sid, len(market_watcher_strategies)
    )