...
INITIAL_DELAY_H = 20        # первый запуск через 20 часов после старта сервиса
...
WEIGHT_CLIP_MAX          = 0.70    # максимальный вес компоненты
...
async def _train_and_activate_weights(strategy_id: int, time_frame: str) -> bool:
    async with infra.pg_pool.acquire() as conn:
        ...
        # оцениваем «важность» признаков на train: берём абсолютную point-biserial корреляцию (упрощённо — кореляция Пирсона с бинарной меткой)
        imp = _feature_importance_corr(X_train, Y_train)  # dict {"R":..., "P":..., "C":..., "S":...}
        ...
        await conn.execute(
            ...
            f'{{"wR":{weights["wR"]:.6f},"wP":{weights["wP"]:.6f},"wC":{weights["wC"]:.6f},"wS":{weights["wS"]:.6f}}}',
