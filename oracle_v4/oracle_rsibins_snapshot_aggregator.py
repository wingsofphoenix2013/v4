# oracle_rsibins_snapshot_aggregator.py — RSI-bins snapshot агрегатор: Этап 1 (каркас, приём закрытий из Stream)
            ...
            for _, records in resp:
                for msg_id, data in records:
                    try:
                        ...
                        # Этап 1: фиксируем только факт получения закрытия
                        log.info("[RSI-BINS SNAP] closed position received: uid=%s", pos_uid)
