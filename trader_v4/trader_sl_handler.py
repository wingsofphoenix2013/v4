...
async def _handle_status_event(raw: Dict[str, Any]) -> None:
    ...
    # остаток фактический (без остатка — ничего переставлять)
    ...
    # найдём активный SL, чтобы взять его фактический объём (если есть)
    sl_qty = await _fetch_active_sl_qty(uid)
    if sl_qty is None:
        # fallback: можно взять left_qty, но лучше не шуметь без явного SL
        log.debug("ℹ️ SL_HANDLER: активный SL не найден для uid=%s — пропускаем перестановку", uid)
        return
