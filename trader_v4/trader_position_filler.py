...
async def _handle_signal_opened(record_id: str, data: Dict[str, Any]) -> None:
    ...
    if not symbol or notional_value <= 0 or quantity_internal is None or quantity_internal <= 0:
        log.info("⚠️ Пустые/некорректные поля (symbol=%s, notional=%s, qty=%s) — пропуск uid=%s",
    ...
    try:
        if config.is_live_trading() and (strategy_id in config.eligible_for_bybit):
            ...
            log.info("📨 BYBIT_INTENT: отправлен create_entry_tpsl uid=%s sym=%s sid=%s", position_uid, symbol, strategy_id)
        else:
            log.info("ℹ️ BYBIT_INTENT: пропуск (live=%s, eligible=%s) uid=%s",
