# bt_signals_macdcross_level2.py — stream-backfill воркер уровня 2 для MACD-cross: создаёт собственный run, формирует membership по good bins (v2)
...
# 🔸 Логгер модуля
log = logging.getLogger("BT_SIG_MACD_CROSS_L2")
...
# 🔸 Публичная точка входа: stream-backfill сигнал уровня 2 для MACD-cross
async def run_macdcross_level2_stream_backfill(
    signal: Dict[str, Any],
    ...
    if not parent_run_info:
        log.warning(
            "BT_SIG_MACD_CROSS_L2: parent run not found — level2_signal_id=%s parent_signal_id=%s scenario_id=%s parent_run_id=%s",
            signal_id,
            parent_signal_id,
            scenario_id,
            parent_run_id,
    ...
    if not analysis_cfg:
        log.debug(
            "BT_SIG_MACD_CROSS_L2: winner analysis not in cache — skip (winner_analysis_id=%s)",
            winner_analysis_id,
        )
        return
    ...
    if family_key == "lr" and analysis_key == "lr_mtf":
        ...
        log.debug(
            "BT_SIG_MACD_CROSS_L2: winner plugin not supported — winner_analysis_id=%s family=%s key=%s winner_param='%s'",
            winner_analysis_id,
            family_key,
            analysis_key,
            str(winner_param),
    ...
    log.debug(
        "BT_SIG_MACD_CROSS_L2: start — level2_signal_id=%s name='%s' pipeline_mode=%s scenario_id=%s parent_signal_id=%s parent_run_id=%s "
    ...
    for symbol in symbols:
        tasks.append(
            asyncio.create_task(
                ...
                name=f"BT_SIG_MACD_CROSS_L2_{pipeline_mode}_{signal_id}_{symbol}",
            )
        )
    ...
    try:
        ...
        error_text = str(e)
        log.error(
            "BT_SIG_MACD_CROSS_L2: failed — level2_signal_id=%s level2_run_id=%s err=%s",
            signal_id,
            level2_run_id,
            e,
            exc_info=True,
    ...
    try:
        ...
        log.error(
            "BT_SIG_MACD_CROSS_L2: failed to publish ready_v2 — signal_id=%s run_id=%s err=%s",
            signal_id,
            level2_run_id,
            e,
            exc_info=True,
