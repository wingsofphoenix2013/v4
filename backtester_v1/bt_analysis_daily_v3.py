# bt_analysis_daily_v3.py — суточная агрегация статистики original/filtered по результатам финального postproc v3
...
# 🔸 Константы стримов и настроек воркера v3
POSTPROC_READY_STREAM_KEY_V3 = "bt:analysis:postproc_ready_v3"

DAILY_CONSUMER_GROUP_V3 = "bt_analysis_daily_v3"
DAILY_CONSUMER_NAME_V3 = "bt_analysis_daily_v3_main"
...
log = logging.getLogger("BT_ANALYSIS_DAILY_V3")


# 🔸 Публичная точка входа: оркестратор суточной статистики v3
async def run_bt_analysis_daily_v3_orchestrator(pg, redis):
    log.debug("BT_ANALYSIS_DAILY_V3: оркестратор запущен")
    ...
    while True:
        try:
            ...
            for stream_key, messages in entries:
                if stream_key != POSTPROC_READY_STREAM_KEY_V3:
                    continue
                ...
                for entry_id, fields in messages:
                    ...
                    task = asyncio.create_task(
                        ...
                        name=f"BT_ANALYSIS_DAILY_V3_{entry_id}",
                    )
                    tasks.append(task)
            ...
            if tasks:
                ...
                log.info(
                    "BT_ANALYSIS_DAILY_V3: обработан пакет сообщений — сообщений=%s, ошибок=%s",
                    total_msgs,
                    errors,
                )

        except Exception as e:
            log.error("BT_ANALYSIS_DAILY_V3: ошибка в основном цикле: %s", e, exc_info=True)
            await asyncio.sleep(2)


# 🔸 Проверка/создание consumer group для стрима bt:analysis:postproc_ready_v3
async def _ensure_consumer_group(redis) -> None:
    try:
        await redis.xgroup_create(
            name=POSTPROC_READY_STREAM_KEY_V3,
            groupname=DAILY_CONSUMER_GROUP_V3,
            id="$",
            mkstream=True,
        )
        log.debug(
            "BT_ANALYSIS_DAILY_V3: создана consumer group '%s' для стрима '%s'",
            DAILY_CONSUMER_GROUP_V3,
        ...
        if "BUSYGROUP" in msg:
            log.debug(
                "BT_ANALYSIS_DAILY_V3: consumer group '%s' для стрима '%s' уже существует",
                DAILY_CONSUMER_GROUP_V3,
                POSTPROC_READY_STREAM_KEY_V3,
            )
        else:
            log.error(
                "BT_ANALYSIS_DAILY_V3: ошибка при создании consumer group '%s': %s",
                DAILY_CONSUMER_GROUP_V3,
                e,
                exc_info=True,
            )
...
# 🔸 Чтение сообщений из стрима bt:analysis:postproc_ready_v3
async def _read_from_stream(redis) -> List[Any]:
    entries = await redis.xreadgroup(
        groupname=DAILY_CONSUMER_GROUP_V3,
        consumername=DAILY_CONSUMER_NAME_V3,
        streams={POSTPROC_READY_STREAM_KEY_V3: ">"},
...
# 🔸 Разбор одного сообщения из стрима bt:analysis:postproc_ready_v3
def _parse_postproc_ready_message(fields: Dict[str, str]) -> Optional[Dict[str, Any]]:
    try:
        ...
        log.error("BT_ANALYSIS_DAILY_V3: ошибка разбора сообщения: %s, fields=%s", e, fields, exc_info=True)
        return None


# 🔸 Обработка одного сообщения из bt:analysis:postproc_ready_v3
async def _process_message(
    ...
    async with sema:
        ...
        if not ctx:
            await redis.xack(POSTPROC_READY_STREAM_KEY_V3, DAILY_CONSUMER_GROUP_V3, entry_id)
            return
        ...
        if last_finished is not None and last_finished == dedup_ts:
            log.debug(
                "BT_ANALYSIS_DAILY_V3: дубликат scenario_id=%s signal_id=%s dedup_ts=%s — пропуск",
                scenario_id,
                signal_id,
                dedup_ts,
            )
        ...
        try:
            ...
            result = await _rebuild_daily_for_pair_v3(
                pg=pg,
                scenario_id=scenario_id,
                signal_id=signal_id,
                deposit=deposit,
            ...
            log.info(
                "BT_ANALYSIS_DAILY_V3: пересчёт завершён — scenario_id=%s signal_id=%s deposit=%s days=%s rows=%s "
            ...
            log.error(
                "BT_ANALYSIS_DAILY_V3: ошибка расчёта scenario_id=%s signal_id=%s: %s",
                scenario_id,
                signal_id,
                e,
                exc_info=True,
...
# 🔸 Пересборка суточной статистики v3 для пары (scenario_id, signal_id) по exit_time::date (UTC)
async def _rebuild_daily_for_pair_v3(
    pg,
    scenario_id: int,
    signal_id: int,
    ...
    # filt по дням (по контейнеру v3)
    filt_rows = await _load_filt_daily_rows_v3(pg, scenario_id, signal_id)
    ...
    async with pg.acquire() as conn:
        async with conn.transaction():
            await conn.execute(
                """
                DELETE FROM bt_analysis_scenario_daily_v3
                WHERE scenario_id = $1
                  AND signal_id   = $2
                """,
                scenario_id,
            ...
            if rows_to_insert:
                await conn.executemany(
                    """
                    INSERT INTO bt_analysis_scenario_daily_v3 (
                        scenario_id,
                        signal_id,
                        day,
                        direction,
...
# 🔸 Загрузка filt-агрегаций по дням из bt_scenario_positions + bt_analysis_positions_postproc_v3
async def _load_filt_daily_rows_v3(pg, scenario_id: int, signal_id: int) -> List[Dict[str, Any]]:
    async with pg.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT
            ...
            JOIN bt_analysis_positions_postproc_v3 pp
