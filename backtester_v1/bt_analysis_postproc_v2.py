# bt_analysis_postproc_v2.py — финальная пост-обработка v2 (логика v3; labels_v2: bad/good/neutral/inactive; good_state=(no bad) AND (has good))
...
# 🔸 Разбор одного сообщения preproc_ready_v2
def _parse_preproc_ready_message(fields: Dict[str, str]) -> Optional[Dict[str, Any]]:
    try:
        ...
        log.error("BT_ANALYSIS_POSTPROC_V2: ошибка разбора сообщения: %s, fields=%s", e, fields, exc_info=True)
        return None
...
async def _process_message(
    ...
    async with sema:
        ...
        if last_finished is not None and last_finished == dedup_ts:
            log.debug(
                "BT_ANALYSIS_POSTPROC_V2: дубликат scenario_id=%s signal_id=%s dedup_ts=%s — пропуск",
                scenario_id,
                signal_id,
                dedup_ts,
            )
        ...
        try:
            ...
            if not model_map:
                log.warning(
                    "BT_ANALYSIS_POSTPROC_V2: model_opt_v2 не найден для scenario_id=%s signal_id=%s — пропуск",
                    scenario_id,
                    signal_id,
                )
                return
            ...
            log.info(
                "BT_ANALYSIS_POSTPROC_V2: scenario_id=%s signal_id=%s — total=%s good=%s bad=%s no_good=%s models=%s elapsed_ms=%s",
                scenario_id,
                signal_id,
            ...
            log.error(
                "BT_ANALYSIS_POSTPROC_V2: ошибка postproc scenario_id=%s signal_id=%s: %s",
                scenario_id,
                signal_id,
                e,
                exc_info=True,
...
# 🔸 Основной постпроцессинг v2 (bad => reject; else require good-hit)
async def _process_pair_postproc_v2(
    ...
    for direction in directions:
        ...
        # грузим bad и good бины из labels_v2
        # inactive и neutral автоматически игнорируются: мы запрашиваем только bad и good
    ...
    # финальное бинарное решение v2 (как v3):
...
# 🔸 Загрузка биннов нужного state из bt_analysis_bins_labels_v2 (+ family_key/key)
...
# 🔸 Обновление bt_analysis_scenario_stat_v2 (orig vs filt + диагностика причин)
...
async def _publish_postproc_ready_v2(
    ...
    try:
        ...
        log.debug(
            "BT_ANALYSIS_POSTPROC_V2: опубликовано событие postproc_ready_v2 scenario_id=%s signal_id=%s",
            scenario_id,
            signal_id,
        )
    except Exception as e:
        log.error(
            "BT_ANALYSIS_POSTPROC_V2: не удалось опубликовать событие scenario_id=%s signal_id=%s: %s",
            scenario_id,
            signal_id,
            e,
            exc_info=True,
