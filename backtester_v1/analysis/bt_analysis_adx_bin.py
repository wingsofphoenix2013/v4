# bt_analysis_adx_bin.py — анализатор распределения позиций по биннам ADX (биннинг берётся из bt_analysis_bins_dict)
...
# 🔸 Логгер модуля
log = logging.getLogger("BT_ANALYSIS_ADX_BIN")


# 🔸 Публичная точка входа анализатора ADX/bin
async def run_adx_bin_analysis(
    ...
    adx_param_name = _get_str_param(params, "param_name", "adx_dmi14_adx")  # adx_dmi14_adx / adx_dmi21_adx

    # загружаем конфигурацию биннов из словаря
    if analysis_id is None:
        log.debug(
            "BT_ANALYSIS_ADX_BIN: analysis_id отсутствует (family=%s, key=%s, name=%s), "
    ...
    if not bins_by_dir:
        log.debug(
            "BT_ANALYSIS_ADX_BIN: нет биннов в bt_analysis_bins_dict для analysis_id=%s, tf=%s "
    ...
    log.debug(
        "BT_ANALYSIS_ADX_BIN: старт анализа id=%s (family=%s, key=%s, name=%s) "
        "для scenario_id=%s, signal_id=%s, tf=%s, adx_param_name=%s, bins_loaded=%s",
        analysis_id,
        family_key,
        analysis_key,
        name,
    ...
    if not positions:
        log.debug(
            "BT_ANALYSIS_ADX_BIN: нет позиций для анализа id=%s (family=%s, key=%s, name=%s), "
    ...
    for p in positions:
        ...
        # извлекаем значение ADX из raw_stat по TF и param_name
        adx_dec = _extract_adx_from_raw_stat(raw_stat, tf, adx_param_name)
        if adx_dec is None:
            positions_skipped += 1
            continue

        # теоретический диапазон ADX [0, 100], клипуем
        if adx_dec < Decimal("0"):
            adx_dec = Decimal("0")
        if adx_dec > Decimal("100"):
            adx_dec = Decimal("100")

        # квантизация до 4 знаков
        adx_dec = _q_decimal(adx_dec)

        bin_name = _assign_bin(bins, adx_dec)
        if bin_name is None:
            positions_skipped += 1
            continue
    ...
    log.debug(
        "BT_ANALYSIS_ADX_BIN: анализатор id=%s (family=%s, key=%s, name=%s), scenario_id=%s, signal_id=%s — "
        "tf=%s, adx_param_name=%s, позиций всего=%s, использовано=%s, пропущено=%s, строк_в_результате=%s",
        analysis_id,
        family_key,
        analysis_key,
        name,
...
async def _load_positions_for_analysis(
    ...
    log.debug(
        "BT_ANALYSIS_ADX_BIN: загружено позиций для анализа scenario_id=%s, signal_id=%s: %s",
        scenario_id,
        signal_id,
        len(positions),
    )
...
# 🔸 Извлечение значения ADX из raw_stat по TF и имени параметра (например, 'adx_dmi14_adx')
def _extract_adx_from_raw_stat(
    raw_stat: Any,
    tf: str,
    adx_param_name: str,
) -> Optional[Decimal]:
    ...
    adx_family = indicators.get("adx_dmi") or {}
    if not isinstance(adx_family, dict):
        return None

    value = adx_family.get(adx_param_name)
    if value is None:
        return None
...
# 🔸 Определение имени бина для значения ADX (границы из bt_analysis_bins_dict)
def _assign_bin(
