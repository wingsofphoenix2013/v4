# bt_analysis_adx_bin.py — анализатор распределения позиций по биннам ADX
...
async def run_adx_bin_analysis(
    ...
    if not bins:
        ...
        bins = _default_adx_bins()

    log.debug(
        "BT_ANALYSIS_ADX_BIN: старт анализа id=%s (family=%s, key=%s, name=%s) "
        "для scenario_id=%s, signal_id=%s, tf=%s, adx_param_name=%s, bins=%s",
        analysis_id,
        family_key,
        analysis_key,
        name,
    ...
    if not positions:
        log.debug(
            "BT_ANALYSIS_ADX_BIN: нет позиций для анализа id=%s, scenario_id=%s, signal_id=%s",
            analysis_id,
            scenario_id,
            signal_id,
        )
    ...
    log.info(
        "BT_ANALYSIS_ADX_BIN: анализатор id=%s (family=%s, key=%s, name=%s), "
...
def _load_bins_from_params(params: Dict[str, Any]) -> List[Dict[str, Decimal]]:
    ...
    try:
        ...
        log.warning("BT_ANALYSIS_ADX_BIN: не удалось распарсить JSON в параметре 'bins', используется дефолтная схема")
        return []
...
# 🔸 Дефолтные бины ADX: 0-10, 10-20, ..., 90-100
def _default_adx_bins() -> List[Dict[str, Decimal]]:
    bins: List[Dict[str, Decimal]] = []
    step = Decimal("10")
...
# 🔸 Определение имени бина для значения ADX
def _assign_bin(
    bins: List[Dict[str, Decimal]],
    value: Decimal,
) -> Optional[str]:
