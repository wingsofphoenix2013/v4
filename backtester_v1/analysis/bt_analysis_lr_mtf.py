# bt_analysis_lr_mtf.py — анализатор распределения позиций по MTF-корзинкам LR (H1/M15 словарь + адаптивные квантили m5 в bt_analysis_bin_dict_adaptive)
...
# 🔸 Публичная точка входа анализатора LR MTF (h1 + m15 + m5, квантили через bt_analysis_bin_dict_adaptive)
async def run_lr_mtf_analysis(
    ...
    if not bins_h1_by_dir or not bins_m15_by_dir:
        log.debug(
            "BT_ANALYSIS_LR_MTF: анализ пропущен (нет биннов H1/M15 в bt_analysis_bins_dict) "
    ...
    # H1/M15: idx 0..5 (из правил LR-канала), m5: rel_m5 ∈ (-∞, +∞) (q6)
    ...
    for p in positions:
        ...
        # индексы бинов H1/M15 по правилам LR-канала
        h1_idx = _lr_position_to_bin_idx(price, h1_upper, h1_lower)
        m15_idx = _lr_position_to_bin_idx(price, m15_upper, m15_lower)

        if h1_idx is None or m15_idx is None:
            positions_skipped += 1
            continue

        # непрерывная позиция внутри/вокруг канала на m5: rel = (price - lower) / (upper - lower)
    ...
    if positions_used == 0:
        log.debug(
            "BT_ANALYSIS_LR_MTF: после фильтрации нет позиций для анализа analysis_id=%s, scenario_id=%s, signal_id=%s "
            "(total=%s, skipped=%s)",
            analysis_id,
            scenario_id,
            signal_id,
    ...
    # группируем по H1-бинам (по нормализованному имени)
    ...
    for h1_bin, group_h in by_h1.items():
        ...
        # H1-группа не прошла порог → всё в H1_bin_X|M15_0|M5_0
        ...
        for m15_bin, group_m15 in by_m15.items():
            ...
            q_groups: Dict[int, List[Decimal]] = {q: [] for q in range(1, LR_MTF_QUANTILES + 1)}
            ...
            for q_idx in range(1, LR_MTF_QUANTILES + 1):
                ...
                to_inclusive = (q_idx == LR_MTF_QUANTILES)
    ...
    try:
        ...
        log.error(
            "BT_ANALYSIS_LR_MTF: ошибка записи bt_analysis_bin_dict_adaptive для analysis_id=%s, scenario_id=%s, signal_id=%s: %s",
            analysis_id,
            scenario_id,
            signal_id,
            e,
    ...
    log.debug(
        "BT_ANALYSIS_LR_MTF: завершено analysis_id=%s (family=%s, key=%s, name=%s), scenario_id=%s, signal_id=%s — "
        "length=%s, min_share=%s, pos_total=%s, pos_used=%s, pos_skipped=%s, rows=%s, H1_groups=%s, adaptive_quantiles=%s",
        analysis_id,
        family_key,
        analysis_key,
        name,
...
# 🔸 Маппинг цены относительно LR-канала в индекс бина 0..5
def _lr_position_to_bin_idx(
    price: float,
    upper: float,
    lower: float,
) -> Optional[int]:
