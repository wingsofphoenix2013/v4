...
# 🔸 Публичная точка входа анализатора RSI/bin
async def run_rsi_bin_analysis(
...
async def _load_positions_for_analysis(
    ...
    log.debug(
        "BT_ANALYSIS_RSI_BIN: загружено позиций для анализа scenario_id=%s, signal_id=%s: %s",
        scenario_id,
        signal_id,
        len(positions),
    )
...
# 🔸 Извлечение значения RSI из raw_stat по TF и имени параметра (например, 'rsi14')
def _extract_rsi_from_raw_stat(
    raw_stat: Any,
    tf: str,
    rsi_param_name: str,
) -> Optional[Decimal]:
    ...
    if not isinstance(rsi_family, dict):
        return None

    value = rsi_family.get(rsi_param_name)
    if value is None:
        return None
