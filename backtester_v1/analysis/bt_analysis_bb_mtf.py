# bt_analysis_bb_mtf.py — анализатор распределения позиций по MTF-корзинкам BB (H1/M15 бины + квантиль m5)
...
# 🔸 Публичная точка входа анализатора BB MTF (h1 + m15 + m5)
async def run_bb_mtf_analysis(
    ...
    log.debug(
        ...
        "для scenario_id=%s, signal_id=%s, min_share=%s, bb_key=%s, params=%s",
        analysis_id,
        family_key,
        analysis_key,
        name,
    ...
    if not positions:
        log.debug(
            "BT_ANALYSIS_BB_MTF: нет позиций для анализа id=%s, scenario_id=%s, signal_id=%s",
            analysis_id,
            scenario_id,
            signal_id,
        )
    ...
    for p in positions:
        ...
        # бины для H1/M15
        h1_bin = _bb_position_to_bin(price, h1_upper, h1_lower)
        m15_bin = _bb_position_to_bin(price, m15_upper, m15_lower)

        if h1_bin is None or m15_bin is None:
            positions_skipped += 1
            continue

        # непрерывная позиция внутри/вокруг BB-канала на m5
    ...
    if positions_used == 0:
        log.debug(
            "BT_ANALYSIS_BB_MTF: после фильтрации нет позиций для анализа scenario_id=%s, signal_id=%s",
            scenario_id,
            signal_id,
        )
        summary = {
    ...
    for h1_bin, group_h in by_h1.items():
        ...
        # H1-бин не прошёл порог → H1_bin_X|M15_bin0|M5_Q0
        ...
        for m15_bin, group_m15 in by_m15.items():
            ...
            for rec in group_m15:
                ...
                # для short инвертируем, чтобы “выгодная” сторона шорта шла в те же квантильные группы
                if direction == "short":
                    sort_key = -rel_f
                else:
                    sort_key = rel_f
...
# 🔸 Маппинг цены относительно BB-канала в дискретный бин bin_0..bin_5
