...
# 🔸 Дефолтные бины EMA-distance: шаг 0.01 снаружи, 0.005 вокруг нуля
def _default_ema_dist_bins() -> List[Dict[str, Decimal]]:
    bins: List[Dict[str, Decimal]] = []

    # ключевые точки (в долях, 0.01 = 1%)
    m5  = Decimal("-0.05")
    m4  = Decimal("-0.04")
    m3  = Decimal("-0.03")
    m2  = Decimal("-0.02")
    ...
    # отрицательные хвосты с шагом 0.01
    bins.append(
        {
            "name": "-0.05--0.04",
            "min": m5,
    ...
    # зона вокруг нуля с шагом 0.005
    bins.append(
        {
            "name": "-0.01--0.005",
            "min": m1,
    ...
    # положительные хвосты с шагом 0.01
    bins.append(
        {
            "name": "0.01-0.02",
            "min": p1,
