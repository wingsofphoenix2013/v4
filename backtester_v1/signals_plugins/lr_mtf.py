# lr_mtf.py — плагин биннинга для анализатора LR MTF (lr50_mtf / lr100_mtf): расчёт bin_name (H1/M15 bins + adaptive quantiles M5) для кандидата сигнала
...
# 🔸 Логгер модуля
log = logging.getLogger("BT_SIG_PLUGIN_LR_MTF")

# 🔸 Единая точность (q6) — как в bt_analysis_lr_mtf.py
Q6 = Decimal("0.000001")
...
# 🔸 LR instance_id overrides (договорённость системы, как в bt_signals_lr_anglemtf.py)
LR_INSTANCE_OVERRIDES: Dict[str, Dict[str, Optional[int]]] = {
    "lr50": {"m5": 7, "m15": 24, "h1": 41},
    "lr100": {"m5": 8, "m15": 25, "h1": 42},
}

# 🔸 Маппинг length -> lr_prefix
LENGTH_TO_PREFIX: Dict[int, str] = {
    50: "lr50",
    100: "lr100",
}
...
# 🔸 Небольшой запас влево для загрузки серий (чтобы не терять первые кандидаты окна)
SERIES_LOOKBACK_MARGIN_MINUTES = 120

# 🔸 Кеш параметров анализаторов: analysis_id -> {"length": int, "min_share": str, "param_name": str, ...}
...
def _floor_to_tf(dt: datetime, tf_minutes: int) -> datetime:
    # dt в UTC, работаем по астрономическим границам
...
# 🔸 Определение LR instance_id по (length, timeframe) через overrides
def resolve_lr_instance_id(length: int, timeframe: str) -> Optional[int]:
    tf = str(timeframe or "").strip().lower()
    prefix = LENGTH_TO_PREFIX.get(int(length) or 0)
    if not prefix:
        return None
...
# 🔸 Загрузка параметров анализатора (length/min_share/param_name)
...
# 🔸 Загрузка серии LR bounds (upper/lower) для одного instance_id и symbol в окне
async def load_lr_bounds_series(
    pg,
    instance_id: int,
    symbol: str,
    from_time: datetime,
    ...
    upper_name = f"lr{int(length)}_upper"
    lower_name = f"lr{int(length)}_lower"
...
# 🔸 Маппинг цены относительно LR-канала в индекс бина 0..5 (как в bt_analysis_lr_mtf.py)
...
# 🔸 Непрерывная позиция внутри/вокруг LR-канала: rel = (price - lower) / (upper - lower)
...
# 🔸 Определение имени бина для значения idx по границам bins_dict (как в bt_analysis_lr_mtf.py)
...
async def init_lr_mtf_plugin_context(
    pg,
    run_id: int,
    scenario_id: int,
    parent_signal_id: int,
    ...
    # условия достаточности
    try:
        length = int(str(params.get("length") or "").strip())
    except Exception:
        length = 0

    if length <= 0:
        raise RuntimeError(f"LR_MTF plugin: analysis_id={analysis_id} has no valid length param")

    lr_prefix = LENGTH_TO_PREFIX.get(length)
    if not lr_prefix:
        raise RuntimeError(f"LR_MTF plugin: unsupported length={length} for analysis_id={analysis_id}")
...
# 🔸 Подготовка серий для одного символа (LR bounds на m5/m15/h1)
async def prepare_symbol_series(
    ...
    m5_iid = resolve_lr_instance_id(length, "m5") or 0
    m15_iid = resolve_lr_instance_id(length, "m15") or 0
    h1_iid = resolve_lr_instance_id(length, "h1") or 0

    # условия достаточности
    if m5_iid <= 0 or m15_iid <= 0 or h1_iid <= 0:
        return {}

    m5_bounds = await load_lr_bounds_series(pg, m5_iid, symbol, window_from, window_to, length)
    m15_bounds = await load_lr_bounds_series(pg, m15_iid, symbol, window_from, window_to, length)
    h1_bounds = await load_lr_bounds_series(pg, h1_iid, symbol, window_from, window_to, length)
...
def compute_lr_mtf_bin_name(
    ...
    if direction != str(plugin_ctx.get("direction") or "").strip().lower():
        # плагин и кандидат должны совпадать по направлению
        return None
    ...
    # индексы H1/M15 по правилам LR-канала
    ...
    rel_m5 = _lr_relative_position(price_f, m5_upper, m5_lower)
    if rel_m5 is None:
        return None
    ...
    # short: инверсия sort_key (как в bt_analysis_lr_mtf.py)
