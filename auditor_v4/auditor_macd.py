# 🔸 auditor_macd.py — аудит «MACD Histogram Regime» по TF m5/m15/h1
...
# 🔸 Логгер
log = logging.getLogger("AUD_MACD")
...
# 🔸 Маски фаз для режимов (бин 1..5 → фаза)
...
# 🔸 Человекочитаемые названия фаз по бин-индексу
...
# 🔸 Точка входа воркера
async def run_auditor_macd():
    # условия достаточности
    if infra.pg_pool is None:
        log.debug("❌ Пропуск auditor_macd: PG не инициализирован")
        return

    # стартовая задержка (если нужна)
    if INITIAL_DELAY_SEC > 0:
        log.debug("⏳ AUD_MACD: ожидание %d сек перед первым запуском", INITIAL_DELAY_SEC)
    ...
    while True:
        try:
            ...
            log.debug("⏹️ AUD_MACD: остановлено по сигналу")
            raise
        except Exception:
            log.exception("❌ AUD_MACD: ошибка прохода — пауза 5 секунд")
            await asyncio.sleep(5)

        log.debug("😴 AUD_MACD: пауза %d сек до следующего запуска", SLEEP_BETWEEN_RUNS_SEC)
...
async def _run_once():
    ...
    log.debug("📦 AUD_MACD: найдено активных MW-стратегий: %d", len(strategies))
    if not strategies:
        return
    ...
    for sid, meta in strategies.items():
        try:
            ...
            log.exception("❌ AUD_MACD: ошибка обработки стратегии sid=%s", sid)


# 🔸 Создать запись в auditor_macd_runs
async def _create_run(now_utc: dt.datetime, win_bounds: Dict[str, Optional[dt.datetime]]) -> int:
    async with infra.pg_pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            INSERT INTO auditor_macd_runs (now_utc, window_7d_from, window_14d_from, window_28d_from)
            VALUES ($1, $2, $3, $4)
            RETURNING id
            """,
            now_utc, win_bounds["7d"], win_bounds["14d"], win_bounds["28d"]
...
async def _process_strategy(
    ...
    if not positions:
        log.debug("ℹ️ AUD_MACD: %s — закрытых позиций нет", title)
        return
    ...
    # снапшоты MACD по TF (m5/m15/h1)
    pos_uids = [p["position_uid"] for p in positions]
    snaps = await _load_macd_snapshots_for_positions(pos_uids)

    # подготовка корзин: data[tf][window][direction] -> [(position_uid, phase_bin, pnl)]
    data: Dict[str, Dict[str, Dict[str, List[Tuple[str, int, float]]]]] = {
        tf: {w: {"long": [], "short": []} for w, _ in WINDOWS} for tf in TIMEFRAMES
    }

    # расчёт фазы и раскладка по корзинам
    for p in positions:
        ...
        for tf in TIMEFRAMES:
            ...
            if not s:
                continue
            phase_bin = _compute_phase_bin_from_snapshot(s, direction)
            if phase_bin is None:
                continue

            for w, _days in WINDOWS:
                if p["in_window"][w]:
                    data[tf][w][direction].append((puid, phase_bin, pnl))
        ...
        for tf in TIMEFRAMES:
            for w, _days in WINDOWS:
                for direction in ("long", "short"):
                    ...
                    for puid, phase_bin, pnl in records:
                        b = int(phase_bin)
                    ...
                    log.debug(
                        '📈 AUD_MACD | %s | TF=%s | dir=%s | window=%s — bins by MACD phase%s',
                    ...
                    for idx in (1, 2, 3, 4, 5):
                        ...
                        phase_name = PHASE_LABELS.get(idx, f"B{idx}")
                        log.debug(
                            "  B%d (%s): N=%d, WR=%.2f%%, ΣPnL=%.6f, ROI=%.4f%%",
                            idx, phase_name, N, WR, pnl_sum, roi
                        )
                    ...
                    # запись в auditor_macd_bin_stats — 5 строк
                    ...
                    for idx in (1, 2, 3, 4, 5):
                        ...
                        await conn.execute(
                            """
                            INSERT INTO auditor_macd_bin_stats
    ...
    for direction in ("long", "short"):
        ...
        log.debug(
            "🏁 AUD_MACD | %s | dir=%s | primary=%s | best=%s | ΔROI=%.2f pp | ROI_sel=%.2f%% | WR_sel=%.2f%% | cov=%.1f%% | conf=%.2f",
            title,
            direction,
            primary_win,
            best_label,
...
async def _publish_best_candidate(
    ...
    if infra.redis_client is None:
        log.debug("ℹ️ AUD_MACD: Redis не инициализирован — пропуск публикации кандидата")
        return
        ...
        fields.update(
            {
                ...
                "source_table": "auditor_macd_mask_results",
    ...
    try:
        ...
        log.debug(
            "📨 AUD_MACD → BEST_SELECTOR | sid=%s dir=%s | variant=%s | eligible=%s",
            sid,
            direction,
            label,
            fields["eligible"],
        ...
        log.exception("❌ AUD_MACD: ошибка публикации кандидата в %s", stream)
...
# 🔸 Подтянуть снапшоты MACD для позиций по всем TF
async def _load_macd_snapshots_for_positions(pos_uids: List[str]) -> Dict[Tuple[str, str], Dict[str, Any]]:
    ...
    async with infra.pg_pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT position_uid, timeframe, param_type, param_base, param_name, value_num, value_text
            FROM indicator_position_stat
            WHERE position_uid = ANY($1)
              AND status = 'ok'
              AND timeframe IN ('m5','m15','h1')
              AND param_type = 'pack'
              AND param_base IN ('macd12','macd') -- при необходимости скорректировать под твой layout
              AND param_name IN ('mode','zero_side','hist_bucket_low_pct','hist_trend_strict','hist_trend_smooth')
            """,
            pos_uids,
        )
    ...
    for r in rows:
        ...
        rec = snaps.setdefault(
            key,
            {
                "mode": None,
                "zero_side": None,
                "hist_bucket_low_pct": None,
                "hist_trend_strict": None,
                "hist_trend_smooth": None,
            },
        )
        param_name = str(r["param_name"])
        if param_name == "mode":
            rec["mode"] = (r["value_text"] or "").strip()
        elif param_name == "zero_side":
            rec["zero_side"] = (r["value_text"] or "").strip()
        elif param_name == "hist_bucket_low_pct":
            rec["hist_bucket_low_pct"] = _to_float_or_none(r["value_num"])
        elif param_name == "hist_trend_strict":
            rec["hist_trend_strict"] = (r["value_text"] or "").strip()
        elif param_name == "hist_trend_smooth":
            rec["hist_trend_smooth"] = (r["value_text"] or "").strip()

    return snaps


# 🔸 Расчёт фазового бина (1..5) по снапшоту MACD
def _compute_phase_bin_from_snapshot(s: Dict[str, Any], direction: str) -> Optional[int]:
    mode = str(s.get("mode") or "").strip().lower()
    zero_side = str(s.get("zero_side") or "").strip().lower()
    hist_trend = str(s.get("hist_trend_smooth") or s.get("hist_trend_strict") or "").strip().lower()
    hb = s.get("hist_bucket_low_pct")

    if hb is None:
        return None

    try:
        size_pct = float(hb) * 100.0  # 0..100
    except Exception:
        return None
    ...
    # согласованность режима с направлением сделки
    align = False
    if mode == "bull" and direction == "long":
        align = True
    if mode == "bear" and direction == "short":
        align = True
    ...
    if align and medium and hist_trend == "rising":
        return 2

    # 3. strong_impulse: большая гистограмма, растёт или стабильна в сторону сделки
    if align and large and hist_trend in ("rising", "stable"):
        return 3

    # 4. late_impulse: большая гистограмма, но уже падает
    if align and large and hist_trend == "falling":
        return 4
    ...
    async with infra.pg_pool.acquire() as conn:
        for direction in ("long", "short"):
            ...
            for w, rec in coverage[direction].items():
                await conn.execute(
                    """
                    INSERT INTO auditor_macd_coverage
...
# 🔸 Вставка строки в auditor_macd_mask_results
async def _insert_mask_result(
    ...
    async with infra.pg_pool.acquire() as conn:
        await conn.execute(
            """
            INSERT INTO auditor_macd_mask_results
            (run_id, strategy_id, direction, window_tag,
