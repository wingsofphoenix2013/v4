# 🔸 auditor_bb.py — аудит «BB Squeeze & Expansion Regime» по TF m5/m15/h1
...
MASK_QBOUNDS: Dict[str, Tuple[Optional[int], Optional[int]]] = {
    # для BB режимов численные границы условные, используются только как метаданные
    "any":  (None, None),
    "low":  (0, 40),
    "mid":  (20, 80),
    "high": (60, 100),
...
# 🔸 Точка входа воркера
async def run_auditor_bb():
    # условия достаточности
    if infra.pg_pool is None:
        log.debug("❌ Пропуск auditor_bb: PG не инициализирован")
        return

    # стартовая задержка (если нужна)
    if INITIAL_DELAY_SEC > 0:
        log.debug("⏳ AUD_BB: ожидание %d сек перед первым запуском", INITIAL_DELAY_SEC)
    ...
    while True:
        try:
            ...
            log.debug("⏹️ AUD_BB: остановлено по сигналу")
            raise
        except Exception:
            log.exception("❌ AUD_BB: ошибка прохода — пауза 5 секунд")
            await asyncio.sleep(5)

        log.debug("😴 AUD_BB: пауза %d сек до следующего запуска", SLEEP_BETWEEN_RUNS_SEC)
...
async def _run_once():
    ...
    log.debug("📦 AUD_BB: найдено активных MW-стратегий: %d", len(strategies))
    if not strategies:
        return
    ...
    for sid, meta in strategies.items():
        try:
            ...
            log.exception("❌ AUD_BB: ошибка обработки стратегии sid=%s", sid)


# 🔸 Создать запись в auditor_bb_runs
async def _create_run(now_utc: dt.datetime, win_bounds: Dict[str, Optional[dt.datetime]]) -> int:
    async with infra.pg_pool.acquire() as conn:
        row = await conn.fetchrow(
            """
            INSERT INTO auditor_bb_runs (now_utc, window_7d_from, window_14d_from, window_28d_from)
            VALUES ($1, $2, $3, $4)
            RETURNING id
            """,
            now_utc, win_bounds["7d"], win_bounds["14d"], win_bounds["28d"]
...
async def _process_strategy(
    ...
    if not positions:
        log.debug("ℹ️ AUD_BB: %s — закрытых позиций нет", title)
        return
    ...
    # снапшоты BB по TF (m5/m15/h1)
    pos_uids = [p["position_uid"] for p in positions]
    snaps = await _load_bb_snapshots_for_positions(pos_uids)
    ...
    for p in positions:
        ...
        for tf in TIMEFRAMES:
            ...
            if not s:
                continue
            phase_bin = _compute_phase_bin_from_snapshot(s)
        ...
        for tf in TIMEFRAMES:
            for w, _days in WINDOWS:
                for direction in ("long", "short"):
                    ...
                    log.debug(
                        '📈 AUD_BB | %s | TF=%s | dir=%s | window=%s — bins by BB phase%s',
                    ...
                    # запись в auditor_bb_bin_stats — 5 строк
                    ...
                    for idx in (1, 2, 3, 4, 5):
                        ...
                        await conn.execute(
                            """
                            INSERT INTO auditor_bb_bin_stats
    ...
    for direction in ("long", "short"):
        ...
        log.debug(
            "🏁 AUD_BB | %s | dir=%s | primary=%s | best=%s | ΔROI=%.2f pp | ROI_sel=%.2f%% | WR_sel=%.2f%% | cov=%.1f%% | conf=%.2f",
            title,
            direction,
            primary_win,
            best_label,
...
async def _publish_best_candidate(
    ...
    if infra.redis_client is None:
        log.debug("ℹ️ AUD_BB: Redis не инициализирован — пропуск публикации кандидата")
        return
        ...
        fields.update(
            {
                ...
                "source_table": "auditor_bb_mask_results",
    ...
    try:
        ...
        log.debug(
            "📨 AUD_BB → BEST_SELECTOR | sid=%s dir=%s | variant=%s | eligible=%s",
            sid,
            direction,
            label,
            fields["eligible"],
        ...
        log.exception("❌ AUD_BB: ошибка публикации кандидата в %s", stream)
...
# 🔸 Подтянуть снапшоты BB для позиций по всем TF
async def _load_bb_snapshots_for_positions(pos_uids: List[str]) -> Dict[Tuple[str, str], Dict[str, Any]]:
    ...
    async with infra.pg_pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            WHERE position_uid = ANY($1)
              ...
              AND param_type = 'pack'
              AND param_base = 'bb'
              AND param_name IN ('bucket','bucket_delta','bw_trend_strict','bw_trend_smooth')
            """,
            pos_uids,
        )
    ...
    for r in rows:
        ...
        rec = snaps.setdefault(
            key,
            {
                "bucket": None,
                "bucket_delta": None,
                "bw_trend_strict": None,
                "bw_trend_smooth": None,
            },
        )
        param_name = str(r["param_name"])
        if param_name == "bucket":
            rec["bucket"] = _to_float_or_none(r["value_num"])
        elif param_name == "bucket_delta":
            rec["bucket_delta"] = (r["value_text"] or "").strip()
        elif param_name == "bw_trend_strict":
            rec["bw_trend_strict"] = (r["value_text"] or "").strip()
        elif param_name == "bw_trend_smooth":
            rec["bw_trend_smooth"] = (r["value_text"] or "").strip()

    return snaps


# 🔸 Расчёт фазового бина (1..5) по снапшоту BB
def _compute_phase_bin_from_snapshot(s: Dict[str, Any]) -> Optional[int]:
    # нормализация bucket 0..11
    bucket_raw = s.get("bucket")
    if bucket_raw is None:
        return None
    try:
        b = int(round(float(bucket_raw)))
    except Exception:
        return None
    b = max(0, min(b, 11))

    # выбор bw_trend: сначала smooth, потом strict
    bw_trend = str(s.get("bw_trend_smooth") or s.get("bw_trend_strict") or "").strip().lower()
    bucket_delta = str(s.get("bucket_delta") or "").strip().lower()
    ...
    # нормализуем bucket_delta к простым признакам
    ...
    async with infra.pg_pool.acquire() as conn:
        for direction in ("long", "short"):
            ...
            for w, rec in coverage[direction].items():
                await conn.execute(
                    """
                    INSERT INTO auditor_bb_coverage
...
# 🔸 Вставка строки в auditor_bb_mask_results
async def _insert_mask_result(
    ...
    async with infra.pg_pool.acquire() as conn:
        await conn.execute(
            """
            INSERT INTO auditor_bb_mask_results
            (run_id, strategy_id, direction, window_tag,
