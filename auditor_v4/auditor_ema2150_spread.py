# 🔸 auditor_ema2150_spread.py — аудит «EMACROSS 21/50: spread режим»
#     — считает spread = abs(EMA21−EMA50)/ATR14 на TF (m5/m15/h1) и окнах (7d/14d/28d/total)
#     — по каждому TF/окну/направлению строит квантильные бины (1..5) и bin-статистику
#     — для каждого символа сохраняет per-symbol границы квантилей в auditor_ema2150_spread_thresholds
#     — формирует три маски (m5_only, m5_m15, m5_m15_h1), выбирает best-of-three по ΔROI/ROI/conf/coverage
...
# 🔸 Один проход: создать run, пройти по стратегиям, записать coverage/bin_stats/thresholds/вердикты
async def _run_once():
            ...
            for w, _days in WINDOWS:
                for direction in ("long", "short"):
                    ...
                    for symbol, triples in symbol_series.items():
                        ...
                        # запись thresholds в auditor_ema2150_spread_thresholds
                        q20, q40, q60, q80 = edges
                        n_samples = len(xs)
                        try:
                            await conn.execute(
                                """
                                INSERT INTO auditor_ema2150_spread_thresholds
                            ...
                            log.exception(
                                "⚠️ AUD_EMA2150: ошибка записи thresholds "
                    ...
                    log.debug(
                        '📈 AUD_EMA2150 | %s | TF=%s | dir=%s | window=%s — bins by spread%s',
...
async def _publish_best_candidate(
    ...
    try:
        ...
        log.debug(
            "📨 AUD_EMA2150 → BEST_SELECTOR | sid=%s dir=%s | variant=%s | eligible=%s",
            sid,
            direction,
            label,
            fields["eligible"],
