...
async def _process_strategy(
    ...
    if not positions:
        log.info("ℹ️ AUD_ATRREG: %s — закрытых позиций нет", title)
        return
...
async def _record_mask_with_validation(
    ...
    log.info(
        ...
        d_roi_pp, metrics_sel_primary["wr_selected_pct"] - metrics_sel_primary["wr_all_pct"], metrics_sel_primary["coverage_pct"]
    )
