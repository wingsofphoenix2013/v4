# live_indicators_h1.py — фоновой воркер публикации «живых» индикаторов h1 в Redis KV (ind_live:*) + L1-кэш в памяти
...
# 🔸 Логгер
log = logging.getLogger("LIVE_H1")


# 🔸 Константы воркера
TF = "h1"                            # фиксированный таймфрейм для Stage-1
...
# 🔸 Обработка одного символа: загрузка DF, вычисление по всем инстансам h1, публикация в Redis и L1
async def _process_symbol(redis,
                          live_cache,  # объект L1-кэша; допускается None
                          symbol: str,
                          precision: int,
                          instances_h1: List[Dict[str, Any]],
    ...
    if df is None or df.empty:
        return {"computed": 0, "written": 0, "errors": 0, "skipped": len(instances_h1)}
    ...
    for inst in instances_h1:
        ...
        try:
            ...
            log.debug(f"LIVE_H1 L1 update skipped for {symbol}: merge error")
    ...
    if live_cache and l1_values:
        try:
            ...
            log.debug(f"LIVE_H1 L1 set error for {symbol}: {e}")

    return {"computed": computed, "written": written, "errors": errors, "skipped": skipped}


# 🔸 Один проход LIVE h1 (без внутренних пауз/задержек) — для секвенсора
async def live_h1_pass(redis,
                       get_instances_by_tf,
                       get_precision,
                       get_active_symbols,
    ...
    # снимок активных символов и инстансов h1
    symbols = list(get_active_symbols()) or []
    instances_h1 = [i for i in get_instances_by_tf(TF)]
    total_instances = len(instances_h1)

    if not symbols or not instances_h1:
        elapsed_ms = int((time.monotonic() - t0) * 1000)
        log.info(
            f"LIVE_H1 PASS done: symbols={len(symbols)} instances={total_instances} "
    ...
    async def _wrap(symbol: str):
        async with sem:
            try:
                ...
                precision = 8
            return await _process_symbol(redis, live_cache, symbol, precision, instances_h1, now_ms)
...
# 🔸 Бесконечный воркер LIVE h1 (для режима без секвенсора)
async def run_live_indicators_h1(pg,
                                 redis,
                                 get_instances_by_tf,
                                 get_precision,
                                 get_active_symbols,
                                 live_cache=None):
    log.debug("LIVE_H1: воркер запущен (Stage-1: только h1, только RAW → Redis ind_live:* + L1)")
