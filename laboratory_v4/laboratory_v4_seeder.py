...
# 🔸 Сидер DMI GapTrend (новый)
async def run_dmigaptrend_seeder():
    async with infra.pg_pool.acquire() as conn:
        existing = await conn.fetchval("SELECT COUNT(*) FROM laboratory_instances_v4 WHERE name LIKE 'DMItrend | %'")
        if existing and existing > 0:
            log.info("Сидер: DMItrend-тесты уже есть (%s шт.), сид не нужен", existing)
            return

    log.info("Сидер: начинаем генерацию DMItrend-тестов")
    ...
    async with infra.pg_pool.acquire() as conn:
        async with conn.transaction():
            for wr in WINRATE_VARIANTS:
                for mt_type, mt_value in MIN_TRADE_VARIANTS:
                    for comps in COMPONENTS:
                        name = make_name_dmigt(comps, mt_type, mt_value, wr)
                        row = await conn.fetchrow(
                            """
                            INSERT INTO laboratory_instances_v4
                              (name, active, min_trade_type, min_trade_value, min_winrate)
                        ...
                        for c in comps:
                            if c == "m5":
                                await conn.execute(
                                    "INSERT INTO laboratory_parameters_v4 (lab_id, test_name, test_type, test_tf, param_spec) "
                                    "VALUES ($1, 'dmigap_trend', 'solo', 'm5',  '{}')",
                                    lab_id,
                                )
                            elif c == "m15":
                                await conn.execute(
                                    "INSERT INTO laboratory_parameters_v4 (lab_id, test_name, test_type, test_tf, param_spec) "
                                    "VALUES ($1, 'dmigap_trend', 'solo', 'm15', '{}')",
                                    lab_id,
                                )
                            elif c == "h1":
                                await conn.execute(
                                    "INSERT INTO laboratory_parameters_v4 (lab_id, test_name, test_type, test_tf, param_spec) "
                                    "VALUES ($1, 'dmigap_trend', 'solo', 'h1',  '{}')",
                                    lab_id,
                                )
                            elif c == "comp":
                                await conn.execute(
                                    "INSERT INTO laboratory_parameters_v4 (lab_id, test_name, test_type, test_tf, param_spec) "
                                    "VALUES ($1, 'dmigap_trend', 'comp', NULL,  '{}')",
                                    lab_id,
                                )
