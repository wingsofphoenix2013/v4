# laboratory_wl_analyzer.py — анализатор WL-порогов (winrate): пересчёт пер-TF/направление по окну 7d, финальный триггер all_ready
...
# 🔸 Финальный триггерный стрим oracle
...
async def run_laboratory_wl_analyzer():
    ...
    try:
        ...
        log.debug("📡 LAB_WL_ANALYZER: создана consumer group для %s", TRIGGER_STREAM)
    except Exception as e:
        if "BUSYGROUP" in str(e):
            pass
        else:
            log.exception("❌ LAB_WL_ANALYZER: ошибка создания consumer group")
            return

    log.debug("🚀 LAB_WL_ANALYZER: слушаю %s", TRIGGER_STREAM)
    ...
    while True:
        try:
            ...
            for _, msgs in resp:
                for msg_id, fields in msgs:
                    try:
                        ...
                        # валидация
                        if master_sid <= 0 or version not in VERSIONS:
                            log.debug("ℹ️ LAB_WL_ANALYZER: пропуск payload=%s", payload)
                            acks.append(msg_id)
                            continue

                        # пересобрать карту (на случай новых head’ов) и таргетно пересчитать
                        mapping = await _build_master_mode_map()
                        await _recompute_by_master_version(mapping, master_sid, version)
            ...
            if acks:
                try:
                    await infra.redis_client.xack(TRIGGER_STREAM, WL_CONSUMER_GROUP, *acks)
                except Exception:
                    log.exception("⚠️ LAB_WL_ANALYZER: ошибка ACK (ids=%s)", acks)
...
# 🔸 Построение карты: (master, version, mode) -> tfs_requested (строка)
async def _build_master_mode_map() -> Dict[Tuple[int, str, str], str]:
    # берём мастеров (есть дублёры), декартим по версиям и режимам; tfs — из последнего head или дефолт
    async with infra.pg_pool.acquire() as conn:
        rows = await conn.fetch(
            """
            WITH clients AS (
            ...
            masters AS (SELECT DISTINCT master_sid FROM clients),
            expected AS (
...
async def _recompute_mapping(mapping: Dict[Tuple[int, str, str], str]):
    ...
    log.debug("✅ LAB_WL_ANALYZER: полный пересчёт завершён (combos=%d)", len(mapping))


# 🔸 Таргетный пересчёт по (master, version)
async def _recompute_by_master_version(mapping: Dict[Tuple[int, str, str], str],
    ...
    log.debug("🔁 LAB_WL_ANALYZER: таргетный пересчёт master=%s version=%s завершён (combos=%d)",
              master_sid, version, len(candidates))


# 🔸 Пересчёт одной тройки (master, version, mode) — по обоим направлениям и всем TF
async def _recompute_for_tuple(master_sid: int, version: str, mode: str, tfs_requested: str):
    ...
    for direction in DIRECTIONS:
        for tf in tfs:
            # выбираем «представителя» на этот TF/направление за 7d
            pick = await _pick_client_for_tf(master_sid, version, mode, direction, tf, win_start)
            if pick is None:
                # нет сделок — очистить активы по обоим источникам
                await _delete_wl_active_if_exists(master_sid, version, mode, direction, tf, "mw")
                await _delete_wl_active_if_exists(master_sid, version, mode, direction, tf, "pack")
                log.debug("🧹 LAB_WL_ANALYZER: нет кандидата за 7d — активы очищены (master=%s ver=%s mode=%s %s tf=%s)",
            ...
            if dep <= 0.0:
                dep = 1.0
                log.warning("⚠️ LAB_WL_ANALYZER: deposit<=0, используем 1.0 (client_sid=%s, master=%s ver=%s mode=%s %s tf=%s)",
                            client_sid, master_sid, version, mode, direction, tf)

            # пересчёт порогов для обоих источников
            for source in SOURCES:
                await _recompute_for_source(
...
# 🔸 Выбор «представителя» ПЕР-TF/направление за последние 7 дней
...
# 🔸 Загрузка среза сделок для источника (возвращает [(wr_source, pnl), ...])
async def _load_positions_slice_wl(client_sid: int, version: str, mode: str, direction: str, tf: str,
    ...
    take_key = "mw" if source == "mw" else "pack"

    for r in rows:
        pnl = float(r["pnl"] or 0.0)
        matches = _parse_json_list(r["mw_matches"] if take_key == "mw" else r["pack_wl_matches"])
        wrs = []
        for m in matches:
            try:
...
# 🔸 Пересчёт порога для одного источника
async def _recompute_for_source(*, master_sid: int, client_sid: int, version: str, mode: str,
                                direction: str, tf: str, source: str, deposit: float,
    ...
    wr_max = max((w for w, _ in rows), default=0.0)

    # условия пустого/невалидного среза
    if positions_total == 0 or Decimal(str(wr_max)) < WR_MIN:
        await _delete_wl_active_if_exists(master_sid, version, mode, direction, tf, source)
        log.debug(
            "🧹 LAB_WL_ANALYZER: пустой/невалидный срез (wr_max=%.3f) — актив %s очищен (master=%s ver=%s mode=%s %s tf=%s)",
            wr_max, source, master_sid, version, mode, direction, tf
        )
        return

    # база на T=0.55
    pnl_sum_base = sum(p for w, p in rows if Decimal(str(w)) >= WR_MIN)
    roi_base = float(pnl_sum_base / deposit) if pnl_sum_base != 0 else 0.0
    ...
    if thresholds and thresholds[-1] != WR_ONE and wr_top == WR_ONE:
    ...
    for T in thresholds:
        ...
        for w, p in rows:
            if w >= T_f:
                n_passed += 1
                pnl_sum += p
        roi_T = float(pnl_sum / deposit) if (deposit > 0 and n_passed > 0) else 0.0
...
# 🔸 Persist: история и актив + апдейт in-memory кэша
async def _persist_wl_analysis_and_active(
    *,
    ...
    async with infra.pg_pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                ...
                float(round(best_threshold, 2)), float(roi_base), float(roi_base), int(positions_total), float(deposit_used),
                int(client_sid), win_start, win_end, computed_at,
            )

    # апдейт in-memory кэша
    ...
    log.debug(
        "LAB_WL_ANALYZER: master=%s ver=%s mode=%s %s tf=%s src=%s -> T*=%.2f ROI=%.6f (base=%.6f, n=%d) [client=%s]",
        master_sid, version, mode, direction, tf, source, float(round(best_threshold, 2)), best_roi, roi_base, positions_total, client_sid
    )


# 🔸 Удаление актива WL и очистка кэша
...
def _next_grid_above(x: Decimal) -> Decimal:
    # следующий узел сетки (0.55 + k*0.01), строго не ниже x, затем плюс один шаг; отрезаем по 1.00
    if x <= WR_MIN:
