# live_pack_m15.py — live-PACK m15 (rsi/mfi/ema/atr/lr/adx_dmi/macd/bb) с использованием L1; публикация «минимального» JSON в pack_live:{indicator}:{symbol}:{tf}:{base}
...
async def pack_m15_pass(redis,
    ...
    if not symbols or not instances_m15:
        ...
        log.debug(f"PACK_M15 PASS done: symbols={len(symbols)} bases=0 written=0 errors=0 hits=0 misses=0 elapsed_ms={elapsed_ms}")
        return
