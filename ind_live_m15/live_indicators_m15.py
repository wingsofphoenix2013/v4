# live_indicators_m15.py — фоновой воркер публикации «живых» индикаторов m15 в Redis KV (ind_live:*) + L1-кэш в памяти
...
# 🔸 Логгер
log = logging.getLogger("LIVE_M15")


# 🔸 Константы воркера
TF = "m15"                            # фиксированный таймфрейм для Stage-1
...
# 🔸 Обработка одного символа: загрузка DF, вычисление по всем инстансам m15, публикация в Redis и L1
async def _process_symbol(redis,
                          live_cache,  # объект L1-кэша; допускается None
                          symbol: str,
                          precision: int,
                          instances_m15: List[Dict[str, Any]],
    ...
    if df is None or df.empty:
        return {"computed": 0, "written": 0, "errors": 0, "skipped": len(instances_m15)}
    ...
    for inst in instances_m15:
        ...
        try:
            ...
            log.debug(f"LIVE_M15 L1 update skipped for {symbol}: merge error")
    ...
    if live_cache and l1_values:
        try:
            ...
            log.debug(f"LIVE_M15 L1 set error for {symbol}: {e}")

    return {"computed": computed, "written": written, "errors": errors, "skipped": skipped}


# 🔸 Один проход LIVE m15 (без внутренних пауз/задержек) — для секвенсора
async def live_m15_pass(redis,
                       get_instances_by_tf,
                       get_precision,
                       get_active_symbols,
    ...
    async def _wrap(symbol: str):
        async with sem:
            try:
                ...
                precision = 8
            return await _process_symbol(redis, live_cache, symbol, precision, instances_m15, now_ms)
...
# 🔸 Бесконечный воркер LIVE m15 (для режима без секвенсора)
async def run_live_indicators_m15(pg,
                                 redis,
                                 get_instances_by_tf,
                                 get_precision,
                                 get_active_symbols,
                                 live_cache=None):
    log.debug("LIVE_M15: воркер запущен (Stage-1: только m15, только RAW → Redis ind_live:* + L1)")
