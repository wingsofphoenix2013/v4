# strategy_212_shortema2.py — стратегия: разрешение по EMA-паттернам (m5 и m15, short)
...
log = logging.getLogger("strategy_212_shortema2")
...
async def _ask_ema_m5_m15_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        "checks": json.dumps([{"kind": "ema_pattern", "timeframes": ["m5", "m15"], "source": "on_demand"}]),
...
class Strategy212Shortema2:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_ema_m5_m15_decision(redis, strategy_id, symbol, direction)
        log.debug(f"[DECISION] strat={strategy_id} {symbol} dir={direction} decision={decision} reason={reason}")

        if decision == "allow":
            return True
        elif decision == "deny":
            return ("ignore", "ema_m5_m15_deny")
        else:
            return ("ignore", f"ema_m5_m15_{reason or 'ignore'}")
