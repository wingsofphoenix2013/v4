# strategy_403_ema50100200uni.py — 🔸 Универсальная стратегия (EMA50 + EMA100 + EMA200): три posfeat-триплета подряд, ORACLE пороги 0.2% / 0.55
...
log = logging.getLogger("strategy_403_ema50100200uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy403Ema50100200uni:
    # Проверка и валидация сигнала (EMA50 → EMA100 → EMA200; все должны пройти пороги)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # последовательная проверка: EMA50 → EMA100 → EMA200
        for ema_len in (50, 100, 200):
            ...
            ok = await self._check_oracle_ema(redis, master_sid, direction, ema_len, triplet, min_closed_required)
            if ok is not True:
