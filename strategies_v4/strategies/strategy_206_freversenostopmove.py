# strategy_206_freversenostopmove.py

import logging
import json
from infra import load_indicators, get_price

log = logging.getLogger("STRATEGY_206_FREVERSENOSTOPMOVE")

class Strategy206Freversenostopmove:
    async def validate_signal(self, signal, context):
        ...
        try:
            ...
            log.debug(f"🔍 [206 FREVERSENOSTOPMOVE] symbol={symbol}, direction={direction}, tf={tf}, "
            ...
            if direction == "long":
                if 55 <= rsi <= 75 and lr_angle > 0.005 and mfi <= 90:
                    return True
                return ("ignore", f"фильтр long не пройден: rsi={rsi}, lr_angle={lr_angle}, mfi={mfi}")

            elif direction == "short":
                if 25 <= rsi <= 45 and lr_angle < -0.005 and mfi >= 10:
                    return True
