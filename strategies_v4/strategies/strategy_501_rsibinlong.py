# strategy_501_rsibinlong.py — 🔸 Лонговая стратегия (RSI14 bins): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_501_rsibinlong")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy501Rsibinlong:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # определяем master_sid по зеркалам (напрамкующий/общий/собственный)
        master_sid = self._resolve_mirrored_sid(strategy_cfg, direction, self_sid)

        # читаем posfeat-комбо мастера с таймаутом (первый сон 5с, затем опрос каждые 2с до 60с)
        ...
        # читаем oracle по триплету мастера
        oracle_key = f"oracle:rsi:comp:{master_sid}:long:rsi14:{triplet}"
        ...
        # читаем направленную базу мастера (closed_long)
