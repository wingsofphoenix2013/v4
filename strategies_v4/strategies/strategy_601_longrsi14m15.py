# strategy_601_longrsi14m15.py — стратегия: разрешение по RSI (m15, length=14) через универсальный решатель
...
log = logging.getLogger("strategy_601_longrsi14m15")

REQUEST_STREAM  = "decision_request"
RESPONSE_STREAM = "decision_response"


# 🔸 Отправить запрос решателю и дождаться ответа (RSI m15 len=14), без таймаута
async def _ask_rsi_m15_len14_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: rsi_bucket, только m15, длина 14, всегда on_demand
        "checks": json.dumps([{
            "kind": "rsi_bucket",
            "timeframes": ["m15"],
            "lengths": {"m15": [14]},
...
class Strategy601Longrsi14m15:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_rsi_m15_len14_decision(redis, strategy_id, symbol, direction)
        log.debug(f"[DECISION] strat={strategy_id} {symbol} dir={direction} decision={decision} reason={reason}")

        if decision == "allow":
            return True
        elif decision == "deny":
            return ("ignore", "rsi_m15_deny")
        else:
            return ("ignore", f"rsi_m15_{reason or 'ignore'}")
