# strategy_701_ema100short.py — 🔸 Шортовая стратегия (EMA100 status): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_701_ema100short")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy701Ema100short:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        oracle_key = f"oracle:ema:comp:{master_sid}:short:ema100:{triplet}"
        ...
        if closed_trades < min_closed_required:
            return ("ignore", f"Not enough closed history for EMA100 triplet {triplet}: required={min_closed_required}, got={closed_trades}")
