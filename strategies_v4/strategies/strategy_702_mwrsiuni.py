# strategy_702_mwrsiuni.py — 🔸 Универсальная стратегия (MW×RSI квартет): live MW-триплет + posfeat m5 RSI-bin, ORACLE пороги 0.15% / 0.55
...
log = logging.getLogger("strategy_702_mwrsiuni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy702Mwrsiuni:
    # Проверка и валидация сигнала (MW×RSI квартет)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # читаем m5-feature из posfeat мастера (RSI bin), с ожиданием 5s + опрос 2s до 60s
        posfeat_key = f"posfeat:{master_sid}:{log_uid}:m5:rsi14_bucket"
        rsi_bin_m5 = await self._await_posfeat_value(redis, posfeat_key, initial_sleep_sec=5, poll_interval_sec=2, max_wait_sec=60)
        if not rsi_bin_m5:
            return ("ignore", f"No posfeat m5 RSI bin for {symbol} (master {master_sid}, log_uid={log_uid})")

        # читаем ORACLE квартет (MW×RSI)
        oracle_key = f"oracle:mw_rsi:quartet:{master_sid}:{direction}:mw:{mw_triplet}:rsi:{rsi_bin_m5}"
        oracle_raw = await self._get_str(redis, oracle_key)
        if not oracle_raw:
            return ("ignore", f"No oracle MW×RSI quartet for {symbol}: mw={mw_triplet}, rsi_m5={rsi_bin_m5}")
        ...
        try:
            ...
            return ("ignore", f"Malformed oracle MW×RSI entry for {symbol}")
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for MW×RSI quartet mw={mw_triplet}, rsi_m5={rsi_bin_m5}: "
        ...
        if winrate < 0.55:
            return (
                "ignore",
                f"Winrate below 0.55 for MW×RSI quartet mw={mw_triplet}, rsi_m5={rsi_bin_m5}: got={winrate:.4f}"
            )
