# strategy_401_longrsi7m5.py — стратегия: разрешение по RSI (m5, length=7) через универсальный решатель
...
log = logging.getLogger("strategy_401_longrsi7m5")
...
async def _ask_rsi_m5_len7_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: rsi_bucket, только m5, длина 7, всегда on_demand
        "checks": json.dumps([{
            "kind": "rsi_bucket",
            "timeframes": ["m5"],
            "lengths": {"m5": [7]},
