...
log = logging.getLogger("strategy_701_universal")

class Strategy701Universal:
    async def validate_signal(self, signal, context):
        ...
        try:
            ...
            if snapshot_id in allowed_snapshots:
                return True
            else:
                return ("ignore", f"snapshot {snapshot_id} не в whitelist [{direction}]")

        except Exception:
            log.exception("❌ Ошибка при проверке snapshot")
            return ("ignore", "ошибка при проверке snapshot")
