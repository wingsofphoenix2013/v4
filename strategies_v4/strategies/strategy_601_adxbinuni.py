# strategy_601_adxbinuni.py — 🔸 Универсальная стратегия (ADX bins): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_601_adxbinuni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy601Adxbinuni:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # ждём posfeat-комбо мастера (ADX bins)
        posfeat_key = f"posfeat:{master_sid}:{log_uid}:adxbin_combo"
        ...
        if not triplet:
            return ("ignore", f"No posfeat ADX bins triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

        # читаем oracle по триплету мастера (по направлению)
        oracle_key = f"oracle:adx:comp:{master_sid}:{direction}:adx:{triplet}"
        oracle_raw = await self._get_str(redis, oracle_key)
        if not oracle_raw:
            return ("ignore", f"No information on ADX bins triplet {triplet} for {symbol}")
        ...
        try:
            ...
            return ("ignore", f"Malformed oracle ADX entry for {symbol}")
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for ADX bins triplet {triplet}: required={min_closed_required}, got={closed_trades}"
            )

        # порог winrate
        if winrate < 0.55:
            return ("ignore", f"Winrate below 0.55 for ADX bins triplet {triplet}: got={winrate:.4f}")
