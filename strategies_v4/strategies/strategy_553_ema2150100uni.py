# strategy_553_ema2150100uni.py — 🔸 Универсальная стратегия (EMA21 + EMA50 + EMA100): три posfeat-триплета подряд, ORACLE пороги 0.2% / 0.55
...
log = logging.getLogger("strategy_553_ema2150100uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy553Ema2150100uni:
    # Проверка и валидация сигнала (EMA21 → EMA50 → EMA100; все должны пройти пороги)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # последовательная проверка: EMA21 → EMA50 → EMA100
        for ema_len in (21, 50, 100):
            trip = await self._await_posfeat_combo(
                redis,
                key=f"posfeat:{master_sid}:{log_uid}:ema{ema_len}_status_combo",
                initial_sleep_sec=5,
                poll_interval_sec=2,
                max_wait_sec=60,
            )
            if not trip:
                return ("ignore", f"No posfeat EMA{ema_len} triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

            ok = await self._check_oracle_ema(redis, master_sid, direction, ema_len, trip, min_closed_required)
            if ok is not True:
                return ok  # ("ignore", reason)
