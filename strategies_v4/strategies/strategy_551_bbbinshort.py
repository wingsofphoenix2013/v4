# strategy_551_bbbinshort.py — 🔸 Шортовая стратегия (BB20/2 entry bins): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_551_bbbinshort")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy551Bbbinshort:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # ждём posfeat-комбо мастера (BB entry bins по entry_price vs BB20/2)
        posfeat_key = f"posfeat:{master_sid}:{log_uid}:bbbin_combo"
        ...
        if not triplet:
            return ("ignore", f"No posfeat BB20/2 triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

        # читаем oracle по триплету мастера (short)
        oracle_key = f"oracle:bb:comp:{master_sid}:short:bb20_2_0:{triplet}"
        oracle_raw = await self._get_str(redis, oracle_key)
        if not oracle_raw:
            return ("ignore", f"No information on BB20/2 triplet {triplet} for {symbol}")
        ...
        try:
            ...
            return ("ignore", f"Malformed oracle BB20/2 entry for {symbol}")
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for BB20/2 triplet {triplet}: required={min_closed_required}, got={closed_trades}"
            )

        # порог winrate
        if winrate < 0.55:
            return ("ignore", f"Winrate below 0.55 for BB20/2 triplet {triplet}: got={winrate:.4f}")
