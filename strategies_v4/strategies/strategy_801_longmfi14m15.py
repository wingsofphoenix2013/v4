# strategy_801_longmfi14m15.py — стратегия: разрешение по MFI (m15, length=14) через универсальный решатель
...
log = logging.getLogger("strategy_801_longmfi14m15")
...
async def _ask_mfi_m15_len14_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: mfi_bucket, только m15, длина 14, всегда on_demand
        "checks": json.dumps([{
            "kind": "mfi_bucket",
            "timeframes": ["m15"],
            "lengths": {"m15": [14]},
