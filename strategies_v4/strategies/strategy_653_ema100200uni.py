# strategy_653_ema100200uni.py — 🔸 Универсальная стратегия (EMA100 + EMA200): два posfeat-триплета подряд, ORACLE пороги 0.2% / 0.55
...
log = logging.getLogger("strategy_653_ema100200uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy653Ema100200uni:
    # Проверка и валидация сигнала (EMA100 затем EMA200; оба должны пройти пороги)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # 2) EMA200
        trip200 = await self._await_posfeat_combo(
            redis,
            key=f"posfeat:{master_sid}:{log_uid}:ema200_status_combo",
            initial_sleep_sec=5,
            poll_interval_sec=2,
            max_wait_sec=60,
        )
        if not trip200:
            return ("ignore", f"No posfeat EMA200 triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

        ok200 = await self._check_oracle_ema(redis, master_sid, direction, 200, trip200, min_closed_required)
        if ok200 is not True:
            return ok200  # ("ignore", reason)
