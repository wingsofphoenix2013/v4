# strategy_801_shortemapatm5.py — стратегия: разрешение по EMA-паттернам (только m5, short)
...
log = logging.getLogger("strategy_801_shortemapatm5")
...
async def _ask_ema_m5_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        "checks": json.dumps([{"kind": "ema_pattern", "timeframes": ["m5"], "source": "on_demand"}]),
...
class Strategy801Shortemapatm5:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_ema_m5_decision(redis, strategy_id, symbol, direction)
        log.debug(f"[DECISION] strat={strategy_id} {symbol} dir={direction} decision={decision} reason={reason}")

        if decision == "allow":
            return True
        elif decision == "deny":
            return ("ignore", "ema_m5_deny")
        else:
            return ("ignore", f"ema_m5_{reason or 'ignore'}")
