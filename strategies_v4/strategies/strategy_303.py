...
log = logging.getLogger("STRATEGY_303")
...
class Strategy303:
    ...
    async def validate_signal(self, signal, context) -> bool | str:
        ...
        log.debug(f"⚙️ [Strategy303] Валидация сигнала: symbol={symbol}, direction={direction}")
        ...
        if note:
            ...
            if redis:
                ...
                try:
                    ...
                    log.warning(f"⚠️ [Strategy303] Ошибка записи в Redis log_queue: {e}")
            return "logged"

        return True
# 🔸 Основной метод запуска стратегии
    async def run(self, signal, context):
        log.debug(f"🚀 [Strategy303] Запуск стратегии на сигнале: symbol={signal['symbol']}, direction={signal['direction']}")
        ...
        if redis:
            ...
            try:
                ...
                log.debug(f"📤 [Strategy303] Сигнал отправлен в strategy_opener_stream")
            except Exception as e:
                log.warning(f"⚠️ [Strategy303] Ошибка при отправке в stream: {e}")