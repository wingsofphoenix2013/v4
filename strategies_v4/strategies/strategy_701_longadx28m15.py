# strategy_701_longadx28m15.py — стратегия: разрешение по ADX (m15, length=28) через универсальный решатель
...
log = logging.getLogger("strategy_701_longadx28m15")

REQUEST_STREAM  = "decision_request"
RESPONSE_STREAM = "decision_response"


# 🔸 Отправить запрос решателю и дождаться ответа (ADX m15 len=28), без таймаута
async def _ask_adx_m15_len28_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: adx_bucket, только m15, длина 28, всегда on_demand
        "checks": json.dumps([{
            "kind": "adx_bucket",
            "timeframes": ["m15"],
            "lengths": {"m15": [28]},
...
class Strategy701Longadx28m15:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_adx_m15_len28_decision(redis, strategy_id, symbol, direction)
        log.debug(f"[DECISION] strat={strategy_id} {symbol} dir={direction} decision={decision} reason={reason}")

        if decision == "allow":
            return True
        elif decision == "deny":
            return ("ignore", "adx_m15_deny")
        else:
            return ("ignore", f"adx_m15_{reason or 'ignore'}")
