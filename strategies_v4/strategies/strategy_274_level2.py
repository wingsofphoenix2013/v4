...
log = logging.getLogger("STRATEGY_274_LEVEL2")

class Strategy274Level2:
    async def validate_signal(self, signal, context):
        ...
        try:
            ...
            log.debug(f"[274] symbol={symbol}, tf={tf}, direction={direction}, price={price}, "
            ...
            open_data = await redis.ts().range(f"ts:{symbol}:{tf}:o", t_ms, t_ms)
            close_data = await redis.ts().range(f"ts:{symbol}:{tf}:c", t_ms, t_ms)
            ...
            log.debug(f"[274:CANDLE] time={t_ms}, open={open_val}, close={close_val}")
            ...
            rsi_data = await redis.ts().range(f"ts_ind:{symbol}:{tf}:rsi14", t2, t1)
            ...
            log.debug(f"[274:RSITREND] rsi_2={rsi_2}, rsi_1={rsi_1}")
            ...
            log.exception("❌ Ошибка в strategy_274_level2")
