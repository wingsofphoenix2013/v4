# strategy_551_ema200uni.py — 🔸 Универсальная стратегия (EMA200 status): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_551_ema200uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy551Ema200uni:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        oracle_key = f"oracle:ema:comp:{master_sid}:{direction}:ema200:{triplet}"
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for EMA200 triplet {triplet}: required={min_closed_required}, got={closed_trades}"
            )
