# strategy_851_longadx14m5.py — стратегия: разрешение по ADX (m5, length=14) через универсальный решатель
...
log = logging.getLogger("strategy_851_longadx14m5")

REQUEST_STREAM  = "decision_request"
RESPONSE_STREAM = "decision_response"


# 🔸 Отправить запрос решателю и дождаться ответа (ADX m5 len=14), без таймаута
async def _ask_adx_m5_len14_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: adx_bucket, только m5, длина 14, всегда on_demand
        "checks": json.dumps([{
            "kind": "adx_bucket",
            "timeframes": ["m5"],
            "lengths": {"m5": [14]},
...
class Strategy851Longadx14m5:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_adx_m5_len14_decision(redis, strategy_id, symbol, direction)
        log.debug(f"[DECISION] strat={strategy_id} {symbol} dir={direction} decision={decision} reason={reason}")

        if decision == "allow":
            return True
        elif decision == "deny":
            return ("ignore", "adx_m5_deny")
        else:
            return ("ignore", f"adx_m5_{reason or 'ignore'}")
