...
log = logging.getLogger("strategy_621_universalst")

class Strategy621Universalst:
        ...
        try:
            ...
            for tf in tfs:
                ...
                if conf_data.get("open_allowed") is not True:
                    return ("ignore", f"🟥 open_allowed=False или отсутствует — strategy_id={strategy_id}, snapshot_id={snapshot_id}, tf={tf}")

            return True  # Все таймфреймы прошли

        except Exception:
            log.exception("❌ Необработанная ошибка в strategy_621_universalst")
    ...
    async def run(self, signal, context):
        ...
        try:
            ...
            log.debug(f"📤 [621] Сигнал отправлен: {payload}")
        except Exception as e:
            log.warning(f"⚠️ [621] Ошибка при отправке сигнала: {e}")