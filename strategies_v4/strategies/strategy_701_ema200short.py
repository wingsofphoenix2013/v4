# strategy_701_ema200short.py — 🔸 Шортовая стратегия (EMA200 status): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_701_ema200short")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy701Ema200short:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # ждём posfeat-комбо мастера (EMA200)
        posfeat_key = f"posfeat:{master_sid}:{log_uid}:ema200_status_combo"
        triplet = await self._await_posfeat_combo(redis, posfeat_key, initial_sleep_sec=5, poll_interval_sec=2, max_wait_sec=60)
        if not triplet:
            return ("ignore", f"No posfeat EMA200 status triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

        # читаем oracle по триплету мастера (short)
        oracle_key = f"oracle:ema:comp:{master_sid}:short:ema200:{triplet}"
        oracle_raw = await self._get_str(redis, oracle_key)
        if not oracle_raw:
            return ("ignore", f"No information on EMA200 status triplet {triplet} for {symbol}")
        ...
        try:
            ...
            return ("ignore", f"Malformed oracle EMA200 entry for {symbol}")

        # направленная база мастера (closed_short)
        ...
        if closed_trades < min_closed_required:
            return ("ignore", f"Not enough closed history for EMA200 triplet {triplet}: required={min_closed_required}, got={closed_trades}")

        # порог winrate
        if winrate < 0.55:
            return ("ignore", f"Winrate below 0.55 for EMA200 triplet {triplet}: got={winrate:.4f}")
