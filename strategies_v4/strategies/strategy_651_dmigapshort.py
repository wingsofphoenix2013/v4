# strategy_651_dmigapshort.py — 🔸 Шортовая стратегия (DMI-gap bins): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_651_dmigapshort")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy651Dmigapshort:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        oracle_key = f"oracle:dmi_gap:comp:{master_sid}:short:gap:{triplet}"
