# strategy_701_longemapatm15.py — стратегия: разрешение по EMA-паттернам (только m15)
...
log = logging.getLogger("strategy_701_longemapatm15")
...
# 🔸 Отправить запрос решателю и дождаться ответа (только m15)
async def _ask_ema_m15_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: ema_pattern, только m15, всегда on_demand
        "checks": json.dumps([{
            "kind": "ema_pattern",
            "timeframes": ["m15"],
            "source": "on_demand"
...
class Strategy701Longemapatm15:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_ema_m15_decision(redis, strategy_id, symbol, direction)
        log.debug(f"[DECISION] strat={strategy_id} {symbol} dir={direction} decision={decision} reason={reason}")

        if decision == "allow":
            return True
        elif decision == "deny":
            return ("ignore", "ema_m15_deny")
        else:
            return ("ignore", f"ema_m15_{reason or 'ignore'}")
