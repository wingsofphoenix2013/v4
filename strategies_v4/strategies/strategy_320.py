...
log = logging.getLogger("STRATEGY_320")
...
class Strategy320:
    ...
    async def validate_signal(self, signal, context) -> bool | str:
        ...
        log.debug(f"⚙️ [Strategy320] Валидация сигнала: symbol={symbol}, direction={direction}")
        ...
        if note:
            ...
            if redis:
                ...
                try:
                    ...
                    log.warning(f"⚠️ [Strategy320] Ошибка записи в Redis log_queue: {e}")
            return "logged"

        return True
# 🔸 Основной метод запуска стратегии
    async def run(self, signal, context):
        log.debug(f"🚀 [Strategy320] Запуск стратегии на сигнале: symbol={signal['symbol']}, direction={signal['direction']}")
        ...
        if redis:
            ...
            try:
                ...
                log.debug(f"📤 [Strategy320] Сигнал отправлен в strategy_opener_stream")
            except Exception as e:
                log.warning(f"⚠️ [Strategy320] Ошибка при отправке в stream: {e}")