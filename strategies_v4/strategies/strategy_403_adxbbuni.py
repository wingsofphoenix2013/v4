# strategy_403_adxbbuni.py — 🔸 Универсальная стратегия (ADX-bin + BB-bin): два posfeat-триплета подряд, ORACLE пороги 0.2% / 0.55
...
log = logging.getLogger("strategy_403_adxbbuni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy403Adxbbuni:
    # 🔸 Проверка и валидация сигнала (ADX → BB; оба должны пройти пороги)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # ADX триплет из posfeat (ожидание 5с + опрос 2с до 60с)
        trip_adx = await self._await_posfeat_combo(
            redis,
            key=f"posfeat:{master_sid}:{log_uid}:adxbin_combo",
            initial_sleep_sec=5,
            poll_interval_sec=2,
            max_wait_sec=60,
        )
        if not trip_adx:
            return ("ignore", f"No posfeat ADX triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

        # проверка ADX по ORACLE
        ok_adx = await self._check_oracle(
            redis=redis,
            oracle_key=f"oracle:adx:comp:{master_sid}:{direction}:adx:{trip_adx}",
            label=f"ADX triplet {trip_adx}",
            min_closed_required=min_closed_required,
        )
        if ok_adx is not True:
            return ok_adx  # ("ignore", reason)

        # BB триплет из posfeat (ожидание 5с + опрос 2с до 60с)
        trip_bb = await self._await_posfeat_combo(
            redis,
            key=f"posfeat:{master_sid}:{log_uid}:bbbin_combo",
            initial_sleep_sec=5,
            poll_interval_sec=2,
            max_wait_sec=60,
        )
        if not trip_bb:
            return ("ignore", f"No posfeat BB triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

        # проверка BB по ORACLE
        ok_bb = await self._check_oracle(
            redis=redis,
            oracle_key=f"oracle:bb:comp:{master_sid}:{direction}:bb20_2_0:{trip_bb}",
            label=f"BB triplet {trip_bb}",
            min_closed_required=min_closed_required,
        )
        if ok_bb is not True:
            return ok_bb  # ("ignore", reason)
