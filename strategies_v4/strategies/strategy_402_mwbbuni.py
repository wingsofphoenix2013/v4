# strategy_402_mwbbuni.py — 🔸 Универсальная стратегия (MW×BB квартет): live MW-триплет + posfeat m5 BB-bin, ORACLE пороги 0.15% / 0.55
...
log = logging.getLogger("strategy_402_mwbbuni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy402Mwbbuni:
    # Проверка и валидация сигнала (MW×BB квартет)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # m5 BB-bin из posfeat мастера (ожидание 5с + опрос 2с до 60с)
        posfeat_key = f"posfeat:{master_sid}:{log_uid}:m5:bbbin"
        bb_bin_m5 = await self._await_posfeat_value(redis, posfeat_key, initial_sleep_sec=5, poll_interval_sec=2, max_wait_sec=60)
        if not bb_bin_m5:
            return ("ignore", f"No posfeat m5 BB bin for {symbol} (master {master_sid}, log_uid={log_uid})")

        # ORACLE квартет MW×BB (entry vs BB20/2.0)
        oracle_key = f"oracle:mw_bb:quartet:{master_sid}:{direction}:mw:{mw_triplet}:bb:{bb_bin_m5}"
        oracle_raw = await self._get_str(redis, oracle_key)
        if not oracle_raw:
            return ("ignore", f"No oracle MW×BB quartet for {symbol}: mw={mw_triplet}, bb_m5={bb_bin_m5}")
        ...
        try:
            ...
            return ("ignore", f"Malformed oracle MW×BB entry for {symbol}")
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for MW×BB quartet mw={mw_triplet}, bb_m5={bb_bin_m5}: "
        ...
        if winrate < 0.55:
            return (
                "ignore",
                f"Winrate below 0.55 for MW×BB quartet mw={mw_triplet}, bb_m5={bb_bin_m5}: got={winrate:.4f}"
            )
