# strategy_551_bbbinuni.py — 🔸 Универсальная стратегия (BB20/2 entry bins): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_551_bbbinuni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy551Bbbinuni:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        oracle_key = f"oracle:bb:comp:{master_sid}:{direction}:bb20_2_0:{triplet}"
