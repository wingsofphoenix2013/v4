# strategy_651_universal.py — двунаправленная стратегия с LAB-гейтингом и выбором мастера по направлению (long/short)
...
# 🔸 Логгер
log = logging.getLogger("strategy_651_universal")
...
LAB_USE_AUDITOR = "true"                # "true" | "false" (строкой)
LAB_USE_ORACLE_BL = "false"             # "true" | "false" (строкой)
LAB_DECISION_MODE = "mw_only"           # валидное значение для БД, логически не используется во 2-м контуре
LAB_VERSION = "v5"                      # валидная версия oracle, формально
LAB_WAIT_TIMEOUT_SEC = 120              # таймаут ожидания ответа

# 🔸 Класс стратегии
class Strategy651Universal:
    ...
    async def validate_signal(self, signal, context):
        ...
        request = {
            ...
            "use_auditor": LAB_USE_AUDITOR,           # "true" | "false" (строкой)            
        }
        ...
        try:
            ...
            log.debug(
                "📤 [LAB_REQ_SENT] req=%s client_sid=%s master_sid=%s %s %s tfs=%s mode=%s ver=%s auditor=%s oracle_bl=%s",
                req_uid, client_sid, master_sid, symbol, direction,
                LAB_TIMEFRAMES, LAB_DECISION_MODE, LAB_VERSION, LAB_USE_AUDITOR, LAB_USE_ORACLE_BL,
            )
