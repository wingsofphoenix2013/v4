# strategy_511_shortm5m15h1.py — зеркальная стратегия (шорт; laboratory_v4 TF: m5,m15,h1; ожидание по last-id, таймаут 90с; INFO-логи; запись ignore в signal_log_queue)
...
class Strategy511Shortm5m15h1:
    ...
    async def _wait_for_response(self, redis, req_id: str, since_id: str, timeout_seconds: int = 90):
        ...
        while True:
            ...
            entries = await redis.xread({stream: read_id}, block=1000, count=51)
            if not entries:
                continue
