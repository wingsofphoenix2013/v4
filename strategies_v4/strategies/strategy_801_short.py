# strategy_801_short.py — 🔸 Шортовая стратегия-транзит: пропускает только short без фильтров

import logging
import json
from typing import Any, Dict, Tuple, Union

log = logging.getLogger("strategy_801_short")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy801Short:
    # Проверка и валидация сигнала (только short)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        if direction == "short":
            return True
        if direction == "long":
            return ("ignore", "long signals are disabled")
