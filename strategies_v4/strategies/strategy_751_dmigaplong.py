# strategy_751_dmigaplong.py — 🔸 Лонговая стратегия (DMI-gap bins): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_751_dmigaplong")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy751Dmigaplong:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # ждём posfeat-комбо мастера (DMI-gap bins)
        posfeat_key = f"posfeat:{master_sid}:{log_uid}:dmigapbin_combo"
        ...
        oracle_key = f"oracle:dmi_gap:comp:{master_sid}:long:gap:{triplet}"
        oracle_raw = await self._get_str(redis, oracle_key)
        if not oracle_raw:
            return ("ignore", f"No information on DMI-gap triplet {triplet} for {symbol}")
        ...
        try:
            ...
            return ("ignore", f"Malformed oracle DMI-gap entry for {symbol}")
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for DMI-gap triplet {triplet}: required={min_closed_required}, got={closed_trades}"
            )

        # порог winrate
        if winrate < 0.55:
            return ("ignore", f"Winrate below 0.55 for DMI-gap triplet {triplet}: got={winrate:.4f}")
