# strategy_401_ema9uni.py — 🔸 Универсальная стратегия (EMA9 status): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_401_ema9uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy401Ema9uni:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        oracle_key = f"oracle:ema:comp:{master_sid}:{direction}:ema9:{triplet}"
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for EMA9 triplet {triplet}: required={min_closed_required}, got={closed_trades}"
            )
