# strategy_202_ema100long.py — 🔸 Лонговая стратегия: фильтр по EMA100 status_triplet, зеркало мастера
...
log = logging.getLogger("strategy_202_ema100long")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy202Ema100long:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        oracle_key = f"oracle:ema:comp:{mirrored_sid}:long:ema100:{triplet}"
        ...
        # читаем базу мастера (closed_long)
        ...
        if closed_long is None or closed_long <= 0:
            return ("ignore", f"No directional history: closed_long=0 for master {mirrored_sid}")

        # проверяем объём истории
