...
log = logging.getLogger("strategy_570_long")

class Strategy570Long:
    async def validate_signal(self, signal, context):
        ...
        try:
            ...
            if snapshot_id is None or pattern_id is None:
                return ("ignore", f"нет snapshot_id или pattern_id в ключе {key}")

            # 2. Проверка snapshot
            snap_key = f"confidence:{strategy_id}:long:{tf}:snapshot:{snapshot_id}"
            ...
            # 3. Проверка pattern
            pat_key = f"confidence:{strategy_id}:long:{tf}:pattern:{pattern_id}"
            ...
            log.exception("❌ Ошибка в strategy_570_long")
