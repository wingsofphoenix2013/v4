...
log = logging.getLogger("STRATEGY_21")
...
class Strategy21:
    ...
    async def validate_signal(self, signal, context) -> bool | str:
        ...
        log.debug(f"⚙️ [Strategy21] Валидация сигнала: symbol={symbol}, direction={direction}")
        ...
        try:
            ...
            if not note:
                if rsi is None:
                    ...
                    rsi = float(rsi)
                    if direction == "long" and not (50 < rsi < 70):
                        note = f"отклонено: RSI14 вне диапазона для LONG (rsi={rsi})"
                    elif direction == "short" and not (30 < rsi < 50):
        ...
        if note:
            ...
            if redis:
                ...
                try:
                    ...
                    log.warning(f"⚠️ [Strategy21] Ошибка записи в Redis log_queue: {e}")
            return "logged"

        return True
    # 🔸 Основной метод запуска стратегии
    async def run(self, signal, context):
        log.debug(f"🚀 [Strategy21] Запуск стратегии на сигнале: symbol={signal['symbol']}, direction={signal['direction']}")
        ...
        if redis:
            ...
            try:
                ...
                log.debug(f"📤 [Strategy21] Сигнал отправлен в strategy_opener_stream")
            except Exception as e:
                log.warning(f"⚠️ [Strategy21] Ошибка при отправке в stream: {e}")