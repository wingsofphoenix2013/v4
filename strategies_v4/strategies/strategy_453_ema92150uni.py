# strategy_453_ema92150uni.py — 🔸 Универсальная стратегия (EMA9 + EMA21 + EMA50): три posfeat-триплета подряд, ORACLE пороги 0.2% / 0.55
...
log = logging.getLogger("strategy_453_ema92150uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy453Ema92150uni:
    # Проверка и валидация сигнала (EMA9 → EMA21 → EMA50; все должны пройти пороги)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # последовательная проверка: EMA9 → EMA21 → EMA50
        for ema_len in (9, 21, 50):
