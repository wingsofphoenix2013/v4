# strategy_651_ema50uni.py — 🔸 Универсальная стратегия (EMA50 status): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_651_ema50uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy651Ema50uni:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        oracle_key = f"oracle:ema:comp:{master_sid}:{direction}:ema50:{triplet}"
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for EMA50 triplet {triplet}: required={min_closed_required}, got={closed_trades}"
            )
