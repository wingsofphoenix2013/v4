...
log = logging.getLogger("STRATEGY_55")
...
class Strategy55:
    ...
    async def validate_signal(self, signal, context) -> bool | str:
        ...
        log.debug(f"⚙️ [Strategy55] Валидация сигнала: symbol={symbol}, direction={direction}")
                ...
                if None in [rsi, mfi, adx]:
                    ...
                    if direction == "long":
                        if not (rsi < 40):
                            ...
                            note = f"отклонено: MFI14 >= 45 (mfi={mfi})"
                        elif not (adx < 40):
                        ...
                        if not (rsi > 60):
                            ...
                            note = f"отклонено: MFI14 <= 55 (mfi={mfi})"
                        elif not (adx < 40):
                            note = f"отклонено: ADX >= 40 (adx={adx})"
        ...
        if note:
            ...
            if redis:
                ...
                try:
                    ...
                    log.warning(f"⚠️ [Strategy55] Ошибка записи в Redis log_queue: {e}")
            return "logged"

        return True
    # 🔸 Основной метод запуска стратегии
    async def run(self, signal, context):
        log.debug(f"🚀 [Strategy55] Запуск стратегии на сигнале: symbol={signal['symbol']}, direction={signal['direction']}")
        ...
        if redis:
            ...
            try:
                ...
                log.debug(f"📤 [Strategy55] Сигнал отправлен в strategy_opener_stream")
            except Exception as e:
                log.warning(f"⚠️ [Strategy55] Ошибка при отправке в stream: {e}")