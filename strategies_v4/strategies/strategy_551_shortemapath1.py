# strategy_551_shortemapath1.py — стратегия: разрешение по EMA-паттернам (только h1, short)
...
log = logging.getLogger("strategy_551_shortemapath1")
...
async def _ask_ema_h1_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        "checks": json.dumps([{"kind": "ema_pattern", "timeframes": ["h1"], "source": "on_demand"}]),
...
class Strategy551Shortemapath1:
