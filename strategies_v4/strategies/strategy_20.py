...
log = logging.getLogger("STRATEGY_20")
...
class Strategy20:
    ...
    async def validate_signal(self, signal, context) -> bool | str:
        ...
        log.debug(f"⚙️ [Strategy20] Валидация сигнала: symbol={symbol}, direction={direction}")
        ...
        if note:
            ...
            if redis:
                ...
                try:
                    ...
                    log.warning(f"⚠️ [Strategy20] Ошибка записи в Redis log_queue: {e}")
            return "logged"
    ...
    async def run(self, signal, context):
        log.debug(f"🚀 [Strategy20] Запуск стратегии на сигнале: symbol={signal['symbol']}, direction={signal['direction']}")
        ...
        if redis:
            ...
            try:
                ...
                log.debug(f"📤 [Strategy20] Сигнал отправлен в strategy_opener_stream")
            except Exception as e:
                log.warning(f"⚠️ [Strategy20] Ошибка при отправке в stream: {e}")