# strategy_751_longrsi14h1.py — стратегия: разрешение по RSI (h1, length=14) через универсальный решатель
...
log = logging.getLogger("strategy_751_longrsi14h1")

REQUEST_STREAM  = "decision_request"
RESPONSE_STREAM = "decision_response"


# 🔸 Отправить запрос решателю и дождаться ответа (RSI h1 len=14), без таймаута
async def _ask_rsi_h1_len14_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: rsi_bucket, только h1, длина 14, всегда on_demand
        "checks": json.dumps([{
            "kind": "rsi_bucket",
            "timeframes": ["h1"],
            "lengths": {"h1": [14]},
...
class Strategy751Longrsi14h1:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_rsi_h1_len14_decision(redis, strategy_id, symbol, direction)
        log.debug(f"[DECISION] strat={strategy_id} {symbol} dir={direction} decision={decision} reason={reason}")

        if decision == "allow":
            return True
        elif decision == "deny":
            return ("ignore", "rsi_h1_deny")
        else:
            return ("ignore", f"rsi_h1_{reason or 'ignore'}")
