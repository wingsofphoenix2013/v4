# strategy_451_bbbinlong.py — 🔸 Лонговая стратегия (BB20/2 entry bins): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_451_bbbinlong")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy451Bbbinlong:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # oracle по триплету мастера (long)
        oracle_key = f"oracle:bb:comp:{master_sid}:long:bb20_2_0:{triplet}"
