# strategy_551_longadx14m15.py — стратегия: разрешение по ADX (m15, length=14) через универсальный решатель
...
log = logging.getLogger("strategy_551_longadx14m15")

REQUEST_STREAM  = "decision_request"
RESPONSE_STREAM = "decision_response"


# 🔸 Отправить запрос решателю и дождаться ответа (ADX m15 len=14), без таймаута
async def _ask_adx_m15_len14_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: adx_bucket, только m15, длина 14, всегда on_demand
        "checks": json.dumps([{
            "kind": "adx_bucket",
            "timeframes": ["m15"],
            "lengths": {"m15": [14]},
...
class Strategy551Longadx14m15:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_adx_m15_len14_decision(redis, strategy_id, symbol, direction)
