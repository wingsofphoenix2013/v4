# strategy_752_mwdmigapuni.py — 🔸 Универсальная стратегия (MW×DMI-gap квартет): live MW-триплет + posfeat m5 DMI-gap bin, ORACLE пороги 0.15% / 0.55
...
log = logging.getLogger("strategy_752_mwdmigapuni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy752Mwdmigapuni:
    # Проверка и валидация сигнала (MW×DMI-gap квартет)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # m5 DMI-gap bin из posfeat мастера (ожидание 5с + опрос 2с до 60с)
        posfeat_key = f"posfeat:{master_sid}:{log_uid}:m5:dmigapbin"
        gap_bin_m5 = await self._await_posfeat_value(redis, posfeat_key, initial_sleep_sec=5, poll_interval_sec=2, max_wait_sec=60)
        if not gap_bin_m5:
            return ("ignore", f"No posfeat m5 DMI-gap bin for {symbol} (master {master_sid}, log_uid={log_uid})")

        # ORACLE квартет MW×DMI-gap
        oracle_key = f"oracle:mw_dmigap:quartet:{master_sid}:{direction}:mw:{mw_triplet}:gap:{gap_bin_m5}"
        oracle_raw = await self._get_str(redis, oracle_key)
        if not oracle_raw:
            return ("ignore", f"No oracle MW×DMI-gap quartet for {symbol}: mw={mw_triplet}, gap_m5={gap_bin_m5}")
        ...
        try:
            ...
            return ("ignore", f"Malformed oracle MW×DMI-gap entry for {symbol}")
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for MW×DMI-gap quartet mw={mw_triplet}, gap_m5={gap_bin_m5}: "
        ...
        if winrate < 0.55:
            return (
                "ignore",
                f"Winrate below 0.55 for MW×DMI-gap quartet mw={mw_triplet}, gap_m5={gap_bin_m5}: got={winrate:.4f}"
            )
