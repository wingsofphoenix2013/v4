# strategy_601_rsibinuni.py — 🔸 Универсальная стратегия (RSI14 bins): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_601_rsibinuni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy601Rsibinuni:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # oracle по направлению
        oracle_key = f"oracle:rsi:comp:{master_sid}:{direction}:rsi14:{triplet}"
        ...
        if closed_trades < min_closed_required:
            return ("ignore", f"Not enough closed history for RSI14 triplet {triplet}: required={min_closed_required}, got={closed_trades}")
