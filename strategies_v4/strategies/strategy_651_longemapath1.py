# strategy_651_longemapath1.py — стратегия: разрешение по EMA-паттернам (только h1)
...
log = logging.getLogger("strategy_651_longemapath1")
...
# 🔸 Отправить запрос решателю и дождаться ответа (только h1)
async def _ask_ema_h1_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: ema_pattern, только h1, всегда on_demand
        "checks": json.dumps([{
            "kind": "ema_pattern",
            "timeframes": ["h1"],
            "source": "on_demand"
...
class Strategy651Longemapath1:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_ema_h1_decision(redis, strategy_id, symbol, direction)
        log.debug(f"[DECISION] strat={strategy_id} {symbol} dir={direction} decision={decision} reason={reason}")

        if decision == "allow":
            return True
        elif decision == "deny":
            return ("ignore", "ema_h1_deny")
        else:
            return ("ignore", f"ema_h1_{reason or 'ignore'}")
