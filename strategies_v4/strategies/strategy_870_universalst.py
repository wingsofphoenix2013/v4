...
log = logging.getLogger("strategy_880_universalst")

class Strategy880Universalst:
    async def validate_signal(self, signal, context):
        ...
        try:
            ...
            if winrate_snap is None or winrate_snap <= 0.5:
                return ("ignore", f"🟥 winrate по snapshot {winrate_snap} <= 0.5")
            ...
            if winrate_pat is None or winrate_pat <= 0.5:
                return ("ignore", f"🟥 winrate по pattern {winrate_pat} <= 0.5")
            ...
            log.exception("❌ Ошибка в strategy_880_universalst")
    ...
    async def run(self, signal, context):
        ...
        try:
            ...
            log.debug(f"📤 [880] Сигнал отправлен: {payload}")
        except Exception as e:
            log.warning(f"⚠️ [880] Ошибка при отправке сигнала: {e}")