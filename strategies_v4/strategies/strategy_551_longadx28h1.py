# strategy_551_longadx28h1.py — стратегия: разрешение по ADX (h1, length=28) через универсальный решатель
...
log = logging.getLogger("strategy_551_longadx28h1")
...
async def _ask_adx_h1_len28_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: adx_bucket, только h1, длина 28, всегда on_demand
        "checks": json.dumps([{
            "kind": "adx_bucket",
            "timeframes": ["h1"],
            "lengths": {"h1": [28]},
