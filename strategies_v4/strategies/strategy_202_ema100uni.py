# strategy_202_ema100uni.py — 🔸 Универсальная стратегия: фильтр по EMA100 status_triplet, зеркало мастера по long/short
...
log = logging.getLogger("strategy_202_ema100uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy202Ema100uni:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # читаем oracle-данные по мастеру (направление учитывается)
        oracle_key = f"oracle:ema:comp:{mirrored_sid}:{direction}:ema100:{triplet}"
        ...
        # читаем базу мастера (по направлению)
        ...
        if closed_dir_total is None or closed_dir_total <= 0:
            return ("ignore", f"No directional history: {closed_field}=0 for master {mirrored_sid}")
    ...
    # 🔸 Определение зеркала мастера (использует *_long / *_short; fallback на market_mirrow или свой id)
