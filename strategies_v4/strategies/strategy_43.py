...
log = logging.getLogger("STRATEGY_43")
...
class Strategy43:
    ...
    async def validate_signal(self, signal, context) -> bool | str:
        ...
        log.debug(f"⚙️ [Strategy43] Валидация сигнала: symbol={symbol}, direction={direction}")
        ...
        try:
            ...
            if price_raw is None:
                ...
                bb_upper = indicators.get("bb20_1_5_upper")
                bb_lower = indicators.get("bb20_1_5_lower")
        ...
        if note:
            ...
            if redis:
                ...
                try:
                    ...
                    log.warning(f"⚠️ [Strategy43] Ошибка записи в Redis log_queue: {e}")
            return "logged"

        return True
    # 🔸 Основной метод запуска стратегии
    async def run(self, signal, context):
        log.debug(f"🚀 [Strategy43] Запуск стратегии на сигнале: symbol={signal['symbol']}, direction={signal['direction']}")
        ...
        if redis:
            ...
            try:
                ...
                log.debug(f"📤 [Strategy43] Сигнал отправлен в strategy_opener_stream")
            except Exception as e:
                log.warning(f"⚠️ [Strategy43] Ошибка при отправке в stream: {e}")