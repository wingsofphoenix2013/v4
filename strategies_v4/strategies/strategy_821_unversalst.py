...
log = logging.getLogger("strategy_821_universalst")

class Strategy821Universalst:
        ...
        try:
            ...
            for tf in tfs:
                ...
                if conf_data.get("open_allowed") is not True:
                    return ("ignore", f"🟥 open_allowed=False — strategy_id={strategy_id}, snapshot_id={snapshot_id}, tf={tf}")

            # ✅ BB-фильтр по последнему tf (m15)
            tf_bb = tfs[-1]
            ...
            log.exception("❌ Ошибка в strategy_821_universalst")
    ...
    async def run(self, signal, context):
        ...
        try:
            ...
            log.debug(f"📤 [821] Сигнал отправлен: {payload}")
        except Exception as e:
            log.warning(f"⚠️ [821] Ошибка при отправке сигнала: {e}")