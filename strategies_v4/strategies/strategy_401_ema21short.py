# strategy_401_ema21short.py — 🔸 Шортовая стратегия (EMA21 status): берём posfeat-комбо мастера, сверяем с ORACLE, пороги по направлению
...
log = logging.getLogger("strategy_401_ema21short")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy401Ema21short:
    ...
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # ждём posfeat-комбо мастера (EMA21)
        posfeat_key = f"posfeat:{master_sid}:{log_uid}:ema21_status_combo"
        triplet = await self._await_posfeat_combo(redis, posfeat_key, initial_sleep_sec=5, poll_interval_sec=2, max_wait_sec=60)
        if not triplet:
            return ("ignore", f"No posfeat EMA21 status triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

        # читаем oracle по триплету мастера (short)
        oracle_key = f"oracle:ema:comp:{master_sid}:short:ema21:{triplet}"
        oracle_raw = await self._get_str(redis, oracle_key)
        if not oracle_raw:
            return ("ignore", f"No information on EMA21 status triplet {triplet} for {symbol}")
        ...
        try:
            ...
            return ("ignore", f"Malformed oracle EMA21 entry for {symbol}")
        ...
        if closed_trades < min_closed_required:
            return ("ignore", f"Not enough closed history for EMA21 triplet {triplet}: required={min_closed_required}, got={closed_trades}")

        # порог winrate
        if winrate < 0.55:
            return ("ignore", f"Winrate below 0.55 for EMA21 triplet {triplet}: got={winrate:.4f}")
