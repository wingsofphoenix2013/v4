# strategy_551_longrsi21h1.py — стратегия: разрешение по RSI (h1, length=21) через универсальный решатель
...
log = logging.getLogger("strategy_551_longrsi21h1")

REQUEST_STREAM  = "decision_request"
RESPONSE_STREAM = "decision_response"


# 🔸 Отправить запрос решателю и дождаться ответа (RSI h1 len=21), без таймаута
async def _ask_rsi_h1_len21_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: rsi_bucket, только h1, длина 21, всегда on_demand
        "checks": json.dumps([{
            "kind": "rsi_bucket",
            "timeframes": ["h1"],
            "lengths": {"h1": [21]},
...
class Strategy551Longrsi21h1:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_rsi_h1_len21_decision(redis, strategy_id, symbol, direction)
