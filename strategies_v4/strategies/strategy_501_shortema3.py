...
log = logging.getLogger("strategy_501_shortema3")

class Strategy501Shortema3:
    async def validate_signal(self, signal, context):
        ...
        # 1) Текущий EMA-триплет (live) для EMA9 в формате "m5-m15-h1" (например, "1-0-3")
        live_key = f"ind_live:{symbol}:ema9_status_triplet"
        try:
        ...
        # 2) Историческая статистика по этому же триплету из oracle (EMA len = 9) для SHORT
        base_id = strategy_cfg.get("market_mirrow") or strategy_cfg["id"]
        oracle_key = f"oracle:emastat:comp:{base_id}:{direction}:9:{triplet}"
