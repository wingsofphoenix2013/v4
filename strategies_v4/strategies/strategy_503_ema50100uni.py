# strategy_503_ema50100uni.py — 🔸 Универсальная стратегия (EMA50 + EMA100): два posfeat-триплета подряд, ORACLE пороги 0.2% / 0.55
...
log = logging.getLogger("strategy_503_ema50100uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy503Ema50100uni:
    # Проверка и валидация сигнала (EMA50 затем EMA100; оба должны пройти пороги)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # 1) EMA50
        trip50 = await self._await_posfeat_combo(
            redis,
            key=f"posfeat:{master_sid}:{log_uid}:ema50_status_combo",
            initial_sleep_sec=5,
            poll_interval_sec=2,
            max_wait_sec=60,
        )
        if not trip50:
            return ("ignore", f"No posfeat EMA50 triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

        ok50 = await self._check_oracle_ema(redis, master_sid, direction, 50, trip50, min_closed_required)
        if ok50 is not True:
            return ok50  # ("ignore", reason)

        # 2) EMA100
        trip100 = await self._await_posfeat_combo(
            redis,
            key=f"posfeat:{master_sid}:{log_uid}:ema100_status_combo",
            initial_sleep_sec=5,
            poll_interval_sec=2,
            max_wait_sec=60,
        )
        if not trip100:
            return ("ignore", f"No posfeat EMA100 triplet for {symbol} (master {master_sid}, log_uid={log_uid})")

        ok100 = await self._check_oracle_ema(redis, master_sid, direction, 100, trip100, min_closed_required)
        if ok100 is not True:
            return ok100  # ("ignore", reason)
    ...
    async def _get_int_hash(self, redis, key: str, field: str) -> int | None:
        try:
            raw = await self._safe_hget(redis, key, field)
            if raw is None:
                return None
    ...
    async def _safe_hget(self, redis, key: str, field: str):
