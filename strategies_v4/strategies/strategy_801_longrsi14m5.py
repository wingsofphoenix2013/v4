# strategy_801_longrsi14m5.py — стратегия: разрешение по RSI (m5, length=14) через универсальный решатель
...
log = logging.getLogger("strategy_801_longrsi14m5")

REQUEST_STREAM  = "decision_request"
RESPONSE_STREAM = "decision_response"


# 🔸 Отправить запрос решателю и дождаться ответа (RSI m5 len=14), без таймаута
async def _ask_rsi_m5_len14_decision(redis, strategy_id: int, symbol: str, direction: str) -> tuple[str, str]:
    ...
    payload = {
        ...
        "mirror": "auto",
        # один check: rsi_bucket, только m5, длина 14, всегда on_demand
        "checks": json.dumps([{
            "kind": "rsi_bucket",
            "timeframes": ["m5"],
            "lengths": {"m5": [14]},
...
class Strategy801Longrsi14m5:
    async def validate_signal(self, signal, context):
        ...
        decision, reason = await _ask_rsi_m5_len14_decision(redis, strategy_id, symbol, direction)
