# strategy_652_mwema21uni.py — 🔸 Универсальная стратегия (MW×EMA21 квартет): live MW-триплет + posfeat m5 EMA21-status, ORACLE пороги 0.15% / 0.55
...
log = logging.getLogger("strategy_652_mwema21uni")

IgnoreResult = Tuple[str, str]  # ("ignore", reason)


# 🔸 Класс стратегии
class Strategy652Mwema21uni:
    # Проверка и валидация сигнала (MW×EMA21 квартет)
    async def validate_signal(self, signal: Dict[str, Any], context: Dict[str, Any]) -> Union[bool, IgnoreResult]:
        ...
        # m5 EMA21-status из posfeat мастера (ожидание 5с + опрос 2с до 60с)
        posfeat_key = f"posfeat:{master_sid}:{log_uid}:m5:ema21_status"
        status_code = await self._await_posfeat_value(redis, posfeat_key, initial_sleep_sec=5, poll_interval_sec=2, max_wait_sec=60)
        if not status_code:
            return ("ignore", f"No posfeat m5 EMA21 status for {symbol} (master {master_sid}, log_uid={log_uid})")

        # ORACLE квартет MW×EMA21
        oracle_key = f"oracle:mw_ema:quartet:{master_sid}:{direction}:mw:{mw_triplet}:ema21:{status_code}"
        oracle_raw = await self._get_str(redis, oracle_key)
        if not oracle_raw:
            return ("ignore", f"No oracle MW×EMA21 quartet for {symbol}: mw={mw_triplet}, ema21_m5={status_code}")
        ...
        try:
            ...
            return ("ignore", f"Malformed oracle MW×EMA21 entry for {symbol}")
        ...
        if closed_trades < min_closed_required:
            return (
                "ignore",
                f"Not enough closed history for MW×EMA21 quartet mw={mw_triplet}, ema21_m5={status_code}: "
        ...
        if winrate < 0.55:
            return (
                "ignore",
                f"Winrate below 0.55 for MW×EMA21 quartet mw={mw_triplet}, ema21_m5={status_code}: got={winrate:.4f}"
            )
