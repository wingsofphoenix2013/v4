...
log = logging.getLogger("STRATEGY_25")
...
class Strategy25:
    ...
    async def validate_signal(self, signal, context) -> bool | str:
        ...
        log.debug(f"⚙️ [Strategy25] Валидация сигнала: symbol={symbol}, direction={direction}")
        ...
        try:
            ...
            if not note:
                if rsi is None:
                    ...
                    elif direction == "short" and not (30 < rsi < 70):
        ...
        if note:
            ...
            if redis:
                ...
                try:
                    ...
                    log.warning(f"⚠️ [Strategy25] Ошибка записи в Redis log_queue: {e}")
            return "logged"

        return True
    # 🔸 Основной метод запуска стратегии
    async def run(self, signal, context):
        log.debug(f"🚀 [Strategy25] Запуск стратегии на сигнале: symbol={signal['symbol']}, direction={signal['direction']}")
        ...
        if redis:
            ...
            try:
                ...
                log.debug(f"📤 [Strategy25] Сигнал отправлен в strategy_opener_stream")
            except Exception as e:
                log.warning(f"⚠️ [Strategy25] Ошибка при отправке в stream: {e}")