...
# Гарантированная доставка ответов (consumer-group router), неблокирующая обработка позиций, ACK по готовности,
# бесконечный перезапрос on-demand до готовности данных (никаких промежуточных ошибок no_ohlcv/no_results)
...
# 🔸 Отправка on-demand индикатора с бесконечным перезапросом до готовности
async def wait_indicator_snapshot(
    redis,
    ...
    *,
    ...
    while True:
        ...
        try:
            _msg_id, data = await q.get()  # ждём свой ответ
        finally:
        ...
        if status == "ok":
            try:
                ...
                return {"open_time": open_time, "results": results}
            except Exception:
                log.warning("indicator_response parse error, retry...", exc_info=True)
                await asyncio.sleep(1.0)
                continue

        # любые не-OK статусы считаем временными (например, no_ohlcv) → переспрашиваем
        await asyncio.sleep(1.0)


# 🔸 Отправка on-demand pack/MW с бесконечным перезапросом до готовности
async def wait_pack(
    redis,
        ...
        if status == "ok":
            try:
                ...
                if isinstance(results, list) and results:
                    return results
                # пустой результат — считаем ещё не готово
            except Exception:
                log.warning("gateway_response parse error, retry...", exc_info=True)
        # не готово → переспрашиваем
        await asyncio.sleep(1.0)


# 🔸 Индикаторы: сбор строк для одного инстанса (всегда до успеха)
async def build_rows_for_indicator_instance(
    ...
    *,
    ...
    payload = await wait_indicator_snapshot(
        redis, ind_router,
        symbol=symbol, timeframe=tf, instance_id=int(instance["id"]),
...
# 🔸 Пакеты: сбор строк для одного вида pack (всегда до успеха)
async def build_rows_for_pack_kind(
    ...
    *,
    ...
    results = await wait_pack(
        redis, gw_router,
        symbol=symbol, timeframe=tf, indicator=kind, timestamp_ms=bar_open_ms
    )
...
# 🔸 MarketWatch: сбор строк для одного вида (всегда до успеха, пишем только state)
    ...
    async def run_one(inst):
        async with _process_indicators_tf_sem:
            try:
                ...
                # редкий случай — повторим на следующем шаге TF/позиции (но здесь не зависаем)
                return []
