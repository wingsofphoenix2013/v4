# indicator_position_stath1.py — воркер on-demand снимка при открытии позиции (h1 only; consumer-groups, watchdog, неблокирующий диспетчер)
...
# 🔸 Параметры воркера / параллелизм (h1)
REQUIRED_TFS            = ("h1",)         # только h1
...
CONCURRENCY_PER_H1      = 60              # лимит on-demand запросов по h1
...
# 🔸 Consumer-groups для ответов (h1 изолированно)
IND_RESP_GROUP      = "iv4_possnap_h1_indresp"
GW_RESP_GROUP       = "iv4_possnap_h1_gwresp"
IND_RESP_CONSUMER   = "iv4_possnap_h1_router_ind_1"
GW_RESP_CONSUMER    = "iv4_possnap_h1_router_gw_1"

# 🔸 Consumer-group для входного стрима позиций (изоляция от m5/m15)
POS_OPEN_GROUP      = "iv4_possnap_h1_group"
POS_OPEN_CONSUMER   = "iv4_possnap_h1_consumer_1"

# 🔸 Логгер
log = logging.getLogger("IND_POS_STAT_H1")
...
# 🔸 Ожидаемые базы паков из активных инстансов (по h1)
...
# 🔸 Роутер ответов (consumer-groups): гарантированная доставка в очередь позиции (h1)
async def run_response_router_h1(redis, req_routes: dict, req_lock: asyncio.Lock,
                                 stop_event: asyncio.Event):

    # создать consumer-groups для ответных стримов (h1-изолированные группы)
    try:
        await redis.xgroup_create(INDICATOR_RESP_STREAM, IND_RESP_GROUP, id="$", mkstream=True)
    except Exception as e:
        if "BUSYGROUP" not in str(e):
            log.warning(f"[h1] xgroup_create ind_resp error: {e}")
    try:
        await redis.xgroup_create(GW_RESP_STREAM, GW_RESP_GROUP, id="$", mkstream=True)
    except Exception as e:
        if "BUSYGROUP" not in str(e):
            log.warning(f"[h1] xgroup_create gw_resp error: {e}")
    ...
    while not stop_event.is_set():
        try:
            ...
            log.error(f"[h1] router loop error: {e}", exc_info=True)
            await asyncio.sleep(0.2)

# 🔸 Обработчик одной позиции (только h1; watchdog утерянных req)
async def handle_position_h1(pg, redis, get_instances_by_tf,
                             position_uid: str, strategy_id: int, symbol: str, created_at_iso: str,
                             req_routes: dict, req_lock: asyncio.Lock,
                             h1_semaphore: asyncio.Semaphore):

    instances = [i for i in get_instances_by_tf("h1")]
    expected_bases = build_expected_pack_bases(instances)
    bar_open_ms = to_bar_open_ms(created_at_iso, "h1")
    ...
    while True:
        ...
        if now >= deadline:
            for inst_id, s in ind_ctx.items():
                if s["state"] == "pending":
                    log.info(f"[TIMEOUT h1] IND {symbol}/h1 inst_id={inst_id} {s['indicator']} last_err={s['last_err']}")
            for ind, s in pack_ctx.items():
                if s["state"] in ("pending", "ok_part"):
                    missing = sorted(list(s["expected_bases"] - s["done_bases"]))
                    log.info(f"[TIMEOUT h1] PACK {symbol}/h1 {ind} missing_bases={missing} last_err={s['last_err']}")
            for kind, s in mw_ctx.items():
                if s["state"] == "pending":
                    log.info(f"[TIMEOUT h1] MW {symbol}/h1 {kind} last_err={s['last_err']}")
            break
                    ...
                    if is_pack:
                        if not isinstance(results, list):
                            ...
                            for item in results:
                                ...
                                rows = build_rows_for_pack_response(position_uid, strategy_id, symbol, "h1", base, open_time_iso, p)
                                if rows:
                                    collected_rows.extend(rows)
                        ...
                        if not isinstance(results, list) or not results:
                            ...
                            if state_val is not None:
                                open_time_iso = p.get("open_time") or datetime.utcfromtimestamp(bar_open_ms / 1000).isoformat()
                                rows = [(
                                    position_uid, strategy_id, symbol, "h1",
        ...
        if all_done_h1():
            ...
            unique_rows_tf = await count_unique_rows(pg, position_uid, "h1")
            log.info(
                f"IND_POS_STAT_H1: position={position_uid} {symbol} h1 snapshot complete: "
...
# 🔸 Основной воркер: диспетчер позиций + глобальный роутер ответов (h1 only; неблокирующий)
async def run_indicator_position_stat_h1(pg, redis, get_instances_by_tf, get_precision):
    ...
    try:
        ...
        if "BUSYGROUP" not in str(e):
            log.warning(f"[h1] xgroup_create pos_open error: {e}")

    # роутинг req_id → очередь позиции
    req_routes: dict[str, asyncio.Queue] = {}
    req_lock = asyncio.Lock()

    # лимитер параллельных позиций и запросов h1
    pos_sema = asyncio.Semaphore(POSITIONS_CONCURRENCY)
    h1_semaphore = asyncio.Semaphore(CONCURRENCY_PER_H1)

    # глобальный роутер ответов (consumer-groups)
    stop_event = asyncio.Event()
    router_task = asyncio.create_task(run_response_router_h1(redis, req_routes, req_lock, stop_event))
    ...
    try:
        while True:
            try:
                ...
                log.error(f"[h1] positions read error: {e}", exc_info=True)
                ...
                for msg_id, data in messages:
                    try:
                        ...
                        if not created_at_iso:
                            log.info(f"[h1][SKIP] position {position_uid}: no created_at/received_at")
                            to_ack.append(msg_id)
                            continue

                        async def run_one():
                            async with pos_sema:
                                await handle_position_h1(pg, redis, get_instances_by_tf,
                                                         position_uid, strategy_id, symbol, created_at_iso,
                                                         req_routes, req_lock, h1_semaphore)
                        ...
                        log.error(f"[h1] position spawn error: {e}", exc_info=True)
