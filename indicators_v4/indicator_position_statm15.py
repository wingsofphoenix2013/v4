# indicator_position_statm15.py — воркер on-demand снимка при открытии позиции (m15 only; consumer-groups, watchdog, неблокирующий диспетчер)
...
# 🔸 Параметры воркера / параллелизм (m15)
REQUIRED_TFS            = ("m15",)        # только m15
...
CONCURRENCY_PER_M15     = 80              # лимит on-demand запросов по m15
...
# 🔸 Consumer-groups для ответов (m15 изолированно)
IND_RESP_GROUP      = "iv4_possnap_m15_indresp"
GW_RESP_GROUP       = "iv4_possnap_m15_gwresp"
IND_RESP_CONSUMER   = "iv4_possnap_m15_router_ind_1"
GW_RESP_CONSUMER    = "iv4_possnap_m15_router_gw_1"

# 🔸 Consumer-group для входного стрима позиций (изоляция от m5)
POS_OPEN_GROUP      = "iv4_possnap_m15_group"
POS_OPEN_CONSUMER   = "iv4_possnap_m15_consumer_1"

# 🔸 Логгер
log = logging.getLogger("IND_POS_STAT_M15")
...
# 🔸 Ожидаемые базы паков из активных инстансов (по m15)
...
# 🔸 Роутер ответов (consumer-groups): гарантированная доставка в очередь позиции (m15)
async def run_response_router_m15(redis, req_routes: dict, req_lock: asyncio.Lock,
                                  stop_event: asyncio.Event):

    # создать consumer-groups для ответных стримов (m15-изолированные группы)
    try:
        await redis.xgroup_create(INDICATOR_RESP_STREAM, IND_RESP_GROUP, id="$", mkstream=True)
    except Exception as e:
        if "BUSYGROUP" not in str(e):
            log.warning(f"[m15] xgroup_create ind_resp error: {e}")
    try:
        await redis.xgroup_create(GW_RESP_STREAM, GW_RESP_GROUP, id="$", mkstream=True)
    except Exception as e:
        if "BUSYGROUP" not in str(e):
            log.warning(f"[m15] xgroup_create gw_resp error: {e}")
    ...
    while not stop_event.is_set():
        try:
            ...
            log.error(f"[m15] router loop error: {e}", exc_info=True)
            await asyncio.sleep(0.2)

# 🔸 Обработчик одной позиции (только m15; watchdog утерянных req)
async def handle_position_m15(pg, redis, get_instances_by_tf,
                              position_uid: str, strategy_id: int, symbol: str, created_at_iso: str,
                              req_routes: dict, req_lock: asyncio.Lock,
                              m15_semaphore: asyncio.Semaphore):

    instances = [i for i in get_instances_by_tf("m15")]
    expected_bases = build_expected_pack_bases(instances)
    bar_open_ms = to_bar_open_ms(created_at_iso, "m15")
    ...
    while True:
        ...
        if now >= deadline:
            for inst_id, s in ind_ctx.items():
                if s["state"] == "pending":
                    log.info(f"[TIMEOUT m15] IND {symbol}/m15 inst_id={inst_id} {s['indicator']} last_err={s['last_err']}")
            for ind, s in pack_ctx.items():
                if s["state"] in ("pending", "ok_part"):
                    missing = sorted(list(s["expected_bases"] - s["done_bases"]))
                    log.info(f"[TIMEOUT m15] PACK {symbol}/m15 {ind} missing_bases={missing} last_err={s['last_err']}")
            for kind, s in mw_ctx.items():
                if s["state"] == "pending":
                    log.info(f"[TIMEOUT m15] MW {symbol}/m15 {kind} last_err={s['last_err']}")
            break
                    ...
                    if is_pack:
                        if not isinstance(results, list):
                            ...
                            for item in results:
                                ...
                                rows = build_rows_for_pack_response(position_uid, strategy_id, symbol, "m15", base, open_time_iso, p)
                                if rows:
                                    collected_rows.extend(rows)
                        ...
                        if not isinstance(results, list) or not results:
                            ...
                            item = results[0]
                            base = item.get("base", kind)
                            p = item.get("pack", {})
                            state_val = p.get("state")
                            if state_val is not None:
                                open_time_iso = p.get("open_time") or datetime.utcfromtimestamp(bar_open_ms / 1000).isoformat()
                                rows = [(
                                    position_uid, strategy_id, symbol, "m15",
        ...
        if all_done_m15():
            ...
            unique_rows_tf = await count_unique_rows(pg, position_uid, "m15")
            log.info(
                f"IND_POS_STAT_M15: position={position_uid} {symbol} m15 snapshot complete: "
    ...
    # очистка маршрутов req_id этой позиции (на всякий случай)
    async with req_lock:
        for rid in list(all_req_ids):
            req_routes.pop(rid, None)

# 🔸 Основной воркер: диспетчер позиций + глобальный роутер ответов (m15 only; неблокирующий)
async def run_indicator_position_stat_m15(pg, redis, get_instances_by_tf, get_precision):
    # создать изолированную consumer-group для входного стрима позиций
    try:
        await redis.xgroup_create(POSITIONS_OPEN_STREAM, POS_OPEN_GROUP, id="$", mkstream=True)
    except Exception as e:
        if "BUSYGROUP" not in str(e):
            log.warning(f"[m15] xgroup_create pos_open error: {e}")

    # роутинг req_id → очередь позиции
    req_routes: dict[str, asyncio.Queue] = {}
    req_lock = asyncio.Lock()

    # лимитер параллельных позиций и запросов m15
    pos_sema = asyncio.Semaphore(POSITIONS_CONCURRENCY)
    m15_semaphore = asyncio.Semaphore(CONCURRENCY_PER_M15)

    # глобальный роутер ответов (consumer-groups)
    stop_event = asyncio.Event()
    router_task = asyncio.create_task(run_response_router_m15(redis, req_routes, req_lock, stop_event))

    # пул активных задач позиций
    ...
    try:
        while True:
            try:
                ...
                log.error(f"[m15] positions read error: {e}", exc_info=True)
                ...
                for msg_id, data in messages:
                    try:
                        ...
                        if not created_at_iso:
                            log.info(f"[m15][SKIP] position {position_uid}: no created_at/received_at")
                            to_ack.append(msg_id)
                            continue

                        async def run_one():
                            async with pos_sema:
                                await handle_position_m15(pg, redis, get_instances_by_tf,
                                                          position_uid, strategy_id, symbol, created_at_iso,
                                                          req_routes, req_lock, m15_semaphore)

                        task = asyncio.create_task(run_one())
                        active_tasks.add(task)
                        # неблокирующий диспетчер: ACK сразу после спауна
                        to_ack.append(msg_id)

                    except Exception as e:
                        log.error(f"[m15] position spawn error: {e}", exc_info=True)

            if to_ack:
                await redis.xack(POSITIONS_OPEN_STREAM, POS_OPEN_GROUP, *to_ack)
