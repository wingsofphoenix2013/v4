# packs/atr_bin.py — pack-воркер: бинирование ATR% по адаптивным q6-границам (bt_analysis_bin_dict_adaptive)
...
# 🔸 Единая точность (как в bt_analysis_atr_bin.py)
Q6 = Decimal("0.000001")
...
# 🔸 Pack-воркер ATR% binning
class AtrBinPack:
    # 🔸 Основной метод: вернуть bin_name для ATR% по правилам (val_from/val_to/to_inclusive) с q6
    def bin_value(self, value: Any, rules: list[Any]) -> str | None:
        # условия достаточности
        if not rules:
            return None

        # ожидаем value как dict: {"atr": ..., "price": ...}
        if not isinstance(value, dict):
            return None

        atr_raw = value.get("atr")
        price_raw = value.get("price")

        atr = _safe_decimal(atr_raw)
        price = _safe_decimal(price_raw)

        if atr is None or price is None:
            return None
        if price <= Decimal("0") or atr < Decimal("0"):
            return None

        # ATR% = (atr / price) * 100, q6 ROUND_DOWN
        try:
