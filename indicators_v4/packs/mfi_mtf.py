...
class MfiMtfPack:
    ...
    def mtf_config(self, source_param_name: str) -> dict[str, Any]:
        ...
        return {
            ...
            "component_param": base_param,  # mfi14 / mfi21
        }
    ...
    def bin_candidates(self, values_by_tf: dict[str, Decimal], rules_by_tf: dict[str, list[Any]]) -> list[str]:
        ...
        # клип в диапазон [0, 100] — как в анализаторе
        h1 = max(Decimal("0"), min(Decimal("100"), h1))
        m15 = max(Decimal("0"), min(Decimal("100"), m15))
        m5 = max(Decimal("0"), min(Decimal("100"), m5))
